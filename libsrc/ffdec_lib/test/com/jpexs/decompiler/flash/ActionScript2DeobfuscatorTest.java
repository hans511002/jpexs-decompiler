...
public class ActionScript2DeobfuscatorTest extends ActionScript2TestBase {
	...
	private String recompile(String str) throws ActionParseException,
		...
		ActionList list = ActionListReader.readActionListTimeout(
				new ArrayList<>(), new SWFInputStream(swf, hex),
				SWF.DEFAULT_VERSION, 0, hex.length, "", 1);
	...
	public void testRemoveBasicTrueExpressions(String expression)
			throws ActionParseException, IOException, CompilationException,
	...
	public void testRemoveJumpsToTheNextAction() {
		String actionsString = "ConstantPool \"a\" \"b\" \"c\"\n"
				...
				+ "Jump loc0084\n" + "loc0084:";
		try {
			List<Action> actions = ASMParser.parse(0, true, actionsString,
					swf.version, false);

			DoActionTag doa = getFirstActionTag();
			doa.setActionBytes(Action
					.actionsToBytes(actions, true, swf.version));
			HighlightedTextWriter writer = new HighlightedTextWriter(
					new CodeFormatting(), false);
			Action.actionsToSource(doa, doa.getActions(), "", writer);
			String actualResult = writer.toText();

 ...
 * @author JPEXS
 */
public class ActionDefineFunctionPushRegisterCleanerTest extends
	...
	public void init() throws IOException, InterruptedException {
		...
		swf = new SWF(new BufferedInputStream(new FileInputStream(
				"testdata/as2_definefunc_regs/as2_definefunc_regs.swf")), false);
	}
	...
	private DoInitActionTag getClassSource(String classFullName) {
		for (Tag t : swf.getTags()) {
			if (t instanceof DoInitActionTag) {
				...
				if (exportName != null
						&& exportName.startsWith(SWF.AS2_PKG_PREFIX)) {
					String exportClassName = exportName
							.substring(SWF.AS2_PKG_PREFIX.length());

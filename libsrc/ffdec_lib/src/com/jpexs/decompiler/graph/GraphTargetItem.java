...
public abstract class GraphTargetItem implements Serializable, Cloneable {
	...
	public String toStringNoQuotes(LocalData localData) {
		// todo: honfika: this method should not be called, maybe we should
		// throw an exception
		return toString();
	}
	...
	public GraphTextWriter appendBlock(GraphTargetItem prevLineItem,
		...
		// where there is only one debugline instruction on the beggining.
		...
		for (GraphTargetItem ti : commands) {
			if (!ti.isEmpty()) {
				// Use stored line information if available to place commands on
				// same line
				if (!first
						&& (!useLineInfo || (ti.getLine() < 1 || prevLine < 1 || (prevLine >= 1 && prevLine != ti
								.getLine())))) {
					nwriter.newLine();
				}
				prevLine = ti.getLine();
				first = false;
				ti.toStringSemicoloned(nwriter, localData);
			}
		}
		if (!first) {
			nwriter.newLine();

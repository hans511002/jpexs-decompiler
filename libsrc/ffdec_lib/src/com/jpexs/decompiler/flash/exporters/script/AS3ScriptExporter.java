...
public class AS3ScriptExporter {
	...
	private String handleMxmlMethod(Map<String, String> namespaces, ScriptPack pack, int cindex,
			TraitMethodGetterSetter t) {
		StringBuilder out = new StringBuilder();
		int method = t.method_info;
		try {
			pack.abc.findBody(method).convert(new ConvertData(), "??", ScriptExportMode.AS, false, method,
					pack.scriptIndex, cindex, pack.abc, t, new ScopeStack(), 0/* ? */, new NulWriter(),
											...
											if (sp.object instanceof LocalRegAVM2Item) {
												if (((SetLocalAVM2Item) asg).regIndex == ((LocalRegAVM2Item) sp.object).regIndex) {
													...
													if (sp.propertyName instanceof FullMultinameAVM2Item) {
														String propName = pack.abc.constants
																.getMultiname(
																		((FullMultinameAVM2Item) sp.propertyName).multinameIndex)
																.getName(pack.abc.constants, new ArrayList<>(), true,
																		true);
														if (val instanceof CallPropertyAVM2Item) {
															...
															if (cp.propertyName instanceof FullMultinameAVM2Item) {
																...
																for (Trait ct : pack.abc.instance_info.get(cindex).instance_traits.traits) {
																	if (ct.name_index == name2
																			&& (ct instanceof TraitMethodGetterSetter)) {
																		tagContent.append(handleMxmlMethod(namespaces,
																				pack, cindex,
																				(TraitMethodGetterSetter) ct));
																	}
																}
															}
															...
															props.append(" ").append(propName).append("=\"")
																	.append(EcmaScript.toString(val.getResult()))
																	.append("\"");
														}
													}
												}
	...
	private String handleMxmlArrMethod(Map<String, String> namespaces, ScriptPack pack, int cindex,
			TraitMethodGetterSetter t) {
		StringBuilder out = new StringBuilder();
		int method = t.method_info;
		try {
									...
									if (cp.propertyName instanceof FullMultinameAVM2Item) {
										...
										for (Trait ct : pack.abc.instance_info.get(cindex).instance_traits.traits) {
											if (ct.name_index == name && (ct instanceof TraitMethodGetterSetter)) {
												out.append(handleMxmlMethod(namespaces, pack, cindex,
														(TraitMethodGetterSetter) ct));
											}
										}
									}
	...
	private String generateMxml(ScriptPack pack) {
		...
		for (int ti : pack.traitIndices) {
			...
			if (t instanceof TraitClass) {
				...
				try {
					pack.abc.findBody(iinit).convert(new ConvertData(), "??", ScriptExportMode.AS, false, iinit,
							pack.scriptIndex, cindex, pack.abc, t, new ScopeStack(), 0/* ? */, new NulWriter(),
							new ArrayList<>(), new ArrayList<>(), true);
					...
					for (GraphTargetItem it : iinitBody) {
						if (it instanceof InitPropertyAVM2Item) {
							...
							if (ip.object instanceof ThisAVM2Item) {
								String propName = pack.abc.constants.getMultiname(ip.propertyName.multinameIndex)
										.getName(pack.abc.constants, new ArrayList<>(), true, true);
								GraphTargetItem val = ((InitPropertyAVM2Item) it).value;
								if (val instanceof CallPropertyAVM2Item) {
									CallPropertyAVM2Item cp = (CallPropertyAVM2Item) val;
									if (cp.propertyName instanceof FullMultinameAVM2Item) {
										String subtagName = getTagName(pack, mindex, ip.propertyName.multinameIndex,
												namespaces);
										tagContent.append("<").append(subtagName).append(">");
										int name = ((FullMultinameAVM2Item) cp.propertyName).multinameIndex;
										for (Trait ct : pack.abc.instance_info.get(cindex).instance_traits.traits) {
											if (ct.name_index == name && (ct instanceof TraitMethodGetterSetter)) {
												tagContent.append(handleMxmlMethod(namespaces, pack, cindex,
														(TraitMethodGetterSetter) ct));
											}
										}
										tagContent.append("</").append(subtagName).append(">");
									...
									if (cp.propertyName instanceof FullMultinameAVM2Item) {
										Multiname m = pack.abc.constants
												.getMultiname(((FullMultinameAVM2Item) cp.propertyName).multinameIndex);
										if ("mx.core.DeferredInstanceFromFunction".equals(""
												+ m.getNameWithNamespace(pack.abc.constants, true))) {
											if (!cp.args.isEmpty()) {
												if (cp.args.get(0) instanceof GetPropertyAVM2Item) {
													...
													if (gp.object instanceof ThisAVM2Item) {
														...
														for (Trait ct : pack.abc.instance_info.get(cindex).instance_traits.traits) {
															if (ct.name_index == name
																	&& (ct instanceof TraitMethodGetterSetter)) {
																tagContent.append(handleMxmlArrMethod(namespaces, pack,
																		cindex, (TraitMethodGetterSetter) ct));
															}
														}
													}
												}
									...
									tagProp.append(" ").append(propName).append("=\"")
											.append(EcmaScript.toString(val.getResult())).append("\"");
								}
							}
	...
	public List<File> exportActionScript3(SWF swf, AbortRetryIgnoreHandler handler, String outdir,
			List<ScriptPack> as3scripts, ScriptExportSettings exportSettings, boolean parallel, EventListener evl) {
		...
		for (ScriptPack item : packs) {
			...
			tasks.add(new ExportPackTask(handler, cnt++, packs.size(), item.getClassPath(), item, file, exportSettings,
					parallel, evl));
		}
		...
		if (!parallel || tasks.size() < 2) {
			try {
				...
				logger.log(Level.SEVERE, Helper.formatTimeToText(Configuration.exportTimeout.get())
						+ " ActionScript export limit reached", ex);

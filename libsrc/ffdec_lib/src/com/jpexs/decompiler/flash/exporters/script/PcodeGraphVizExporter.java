...
public class PcodeGraphVizExporter {
	...
	public void exportAs12(ASMSource src, GraphTextWriter writer)
			throws InterruptedException {
		ActionList alist = src.getActions();
		ActionGraph gr = new ActionGraph("", false, alist, new HashMap<>(),
				new HashMap<>(), new HashMap<>(), SWF.DEFAULT_VERSION);
		export(gr, writer);
	}

	public void exportAs3(ABC abc, MethodBody body, GraphTextWriter writer)
			throws InterruptedException {
		AVM2Graph gr = new AVM2Graph(body.getCode(), abc, body, false, -1, -1,
				new HashMap<>(), new ScopeStack(), new HashMap<>(),
				new ArrayList<>(), new HashMap<>(), body.getCode().visitCode(
						body));
		export(gr, writer);
	}

	private void exportGraph(Graph graph, GraphTextWriter writer)
			throws InterruptedException {
		graph.init(null);
		...
		for (GraphPart head : heads) {
			...
			nwriter.append(headName + " [shape=\"circle\"]\r\n");
			nwriter.append(headName + ":s -> "
					+ getBlockName(graphSource, head) + ":n;\r\n");
		}
		for (GraphPart part : allBlocks) {
			...
			for (int j = part.start; j <= part.end; j++) {
				if (j < graphSource.size()) {
					if (knownAddresses
							.contains(graphSource.get(j).getAddress())) {
						blkCodeBuilder
								.append("loc")
								.append(Helper.formatAddress(graphSource.get(j)
										.getAddress())).append(":\r\n");
					}
					String insStr = graphSource.insToString(j);
					if (insStr.length() > INS_LEN_LIMIT) {
						insStr = insStr.substring(0,
								INS_LEN_LIMIT - ELIPSIS.length())
								+ ELIPSIS;
					}
			...
			nwriter.append(partBlockName + " [" + blkStyle + " label=\""
					+ labelStr + "\"];\r\n");
			for (int n = 0; n < part.nextParts.size(); n++) {
				...
				nwriter.append(partBlockName + orientation + " -> "
						+ nextBlockName + ":n;\r\n");
			}
		}
	...
	public void export(Graph graph, GraphTextWriter writer)
			throws InterruptedException {
		GraphTextWriter nwriter = writer.cloneNew();
		nwriter.append("digraph pcode {\r\n");
		exportGraph(graph, nwriter);
		int pos = 0;
		Map<String, Graph> subgraphs = graph.getSubGraphs();
		for (String name : subgraphs.keySet()) {
			nwriter.append("subgraph cluster_" + pos + " {");
			nwriter.append("label=" + new DotId(name, false) + ";\r\n");
			pos++;
			exportGraph(subgraphs.get(name), nwriter);
			nwriter.append("}");
		}
		nwriter.append("}\r\n");
		writer.marge(nwriter);

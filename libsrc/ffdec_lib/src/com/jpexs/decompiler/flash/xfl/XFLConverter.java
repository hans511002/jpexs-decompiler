...
public class XFLConverter {
	...
	private static void convertLineStyle(LINESTYLE ls, int shapeNum, XFLXmlWriter writer) throws XMLStreamException {
		writer.writeStartElement("SolidStroke",
				new String[] { "scaleMode", getScaleMode(ls), "weight", Double.toString(((float) ls.width) / SWF.unitDivisor), });
	...
	private static void convertLineStyle(HashMap<Integer, CharacterTag> characters, LINESTYLE2 ls, int shapeNum, XFLXmlWriter writer)
	...
	private static void convertFillStyle(MATRIX mat, HashMap<Integer, CharacterTag> characters, FILLSTYLE fs, int shapeNum,
	...
	private static boolean shapeHasMultiLayers(HashMap<Integer, CharacterTag> characters, MATRIX mat, int shapeNum,
			List<SHAPERECORD> shapeRecords, FILLSTYLEARRAY fillStyles, LINESTYLEARRAY lineStyles) throws XMLStreamException {
		List<String> layers = getShapeLayers(characters, mat, shapeNum, shapeRecords, fillStyles, lineStyles, false);
		return layers.size() > 1;
	}

	private static void convertShape(HashMap<Integer, CharacterTag> characters, MATRIX mat, int shapeNum, List<SHAPERECORD> shapeRecords,
			FILLSTYLEARRAY fillStyles, LINESTYLEARRAY lineStyles, boolean morphshape, boolean useLayers, XFLXmlWriter writer,
			Map<Integer, String> characterClasses) throws XMLStreamException {
		...
		if (!useLayers) {
			...
			for (int l = layers.size() - 1; l >= 0; l--) {
				...
				writer.writeStartElement("DOMFrame",
						new String[] { "index", "0", "motionTweenScale", "false", "keyMode", Integer.toString(KEY_MODE_SHAPE_LAYERS) });
				writer.writeStartElement("elements");
				writer.writeCharactersRaw(layers.get(l));
				writer.writeCharactersRaw("\n");
				writer.writeEndElement();
				writer.writeEndElement();
				writer.writeEndElement();
				writer.writeEndElement();
	...
	private static List<SHAPERECORD> snapCloseTogether(List<SHAPERECORD> shapeRecords, double maxDistance) {
		...
		for (SHAPERECORD rec : shapeRecords) {
			...
			/*
			 * for (Point p : points) { if (distance(p, currentPoint) <= maxDistance) { currentPoint = (Point) p.clone(); break; } }
			 */
	 ...
	 *
	 * ... straightrecord straightrecord stylechange straightrecord (-2,0) <-- merge this with previous stylegchange
	 *
	 * @param shapeRecords
	 * @return
	 */
	private static List<SHAPERECORD> smoothShape(List<SHAPERECORD> shapeRecords) {
		...
		for (int i = 1; i < ret.size() - 1; i++) {
			if (ret.get(i) instanceof StraightEdgeRecord && (ret.get(i - 1) instanceof StyleChangeRecord)
	...
	private static List<String> getShapeLayers(HashMap<Integer, CharacterTag> characters, MATRIX mat, int shapeNum,
			List<SHAPERECORD> shapeRecords, FILLSTYLEARRAY fillStyles, LINESTYLEARRAY lineStyles, boolean morphshape)
			throws XMLStreamException {
		if (mat == null) {
			mat = new MATRIX();
		}
		...
		for (SHAPERECORD edge : shapeRecords) {
			if (edge instanceof StyleChangeRecord) {
				...
				if (scr.stateFillStyle0) {
					...
					// == 0 ? 0 : fillStylesMap.get(fillStyleCount - lastFillStyleCount + scr.fillStyle0 - 1) + 1;
					if (morphshape) { // ???
				...
				if (scr.stateFillStyle1) {
					...
					// == 0 ? 0 : fillStylesMap.get(fillStyleCount - lastFillStyleCount + scr.fillStyle1 - 1) + 1;
					if (morphshape) {
				...
				if (scr.stateLineStyle) {
					...
					// == 0 ? 0 : lineStyleCount - lastLineStyleCount + scr.lineStyle;
	...
	private static void walkShapeUsages(ReadOnlyTagList timeLineTags, HashMap<Integer, CharacterTag> characters,
			...
			for (int ch : usages.keySet()) {
				if (usages.get(ch) < 2) {
					if (characters.get(ch) instanceof ShapeTag) {
						...
						if (!shapeHasMultiLayers(characters, null, shp.getShapeNum(), shp.getShapes().shapeRecords,
								shp.getShapes().fillStyles, shp.getShapes().lineStyles)) {
							ret.add(ch);
						}
					}
	...
	private static final String[] BLENDMODES = { null, null, "layer", "multiply", "screen", "lighten", "darken", "difference", "add",
	...
	private static void convertSymbolInstance(String name, MATRIX matrix, ColorTransform colorTransform, boolean cacheAsBitmap,
			int blendMode, List<FILTER> filters, boolean isVisible, RGBA backgroundColor, CLIPACTIONS clipActions, Amf3Value metadata,
			CharacterTag tag, HashMap<Integer, CharacterTag> characters, ReadOnlyTagList tags, FLAVersion flaVersion, XFLXmlWriter writer,
			Map<Integer, String> characterClasses) throws XMLStreamException {
		if (matrix == null) {
			matrix = new MATRIX();
		}
		...
		String fileType = getSymbolType(tag);
		String symbolName = fileType + "/Symbol" + tag.getCharacterId();
		if (convertSymbolName) {
			if (tag.getClassName() != null && !tag.getClassName().isEmpty()) {
				symbolName = fileType + "/" + tag.getClassName();
			}
		}
		writer.writeStartElement("DOMSymbolInstance", new String[] { "libraryItemName", symbolName });
		if (name != null) {
		...
		if (metadata != null && (metadata.getValue() instanceof ObjectType)) {
			...
			if (metadataObject.isDynamic()) {
				...
				for (String n : metadataObject.dynamicMembersKeySet()) {
					...
					/*
					 * From JSFL, also data types integerArray ("I"), doubleArray("D") and byteArray("B") can be set. These datatypes can be
					 * in the FLA file but are not exported to SWF with _EMBED_SWF_ publish format.
					 */
				}
	...
	private void convertLibrary(SWF swf, Map<Integer, String> characterVariables, Map<Integer, String> characterClasses,
			Map<Integer, ScriptPack> characterScriptPacks, List<Integer> nonLibraryShapes, String backgroundColor, ReadOnlyTagList tags,
			HashMap<Integer, CharacterTag> characters, HashMap<String, byte[]> files, HashMap<String, byte[]> datfiles,
			FLAVersion flaVersion, XFLXmlWriter writer) throws XMLStreamException {
		...
		for (int ch : characters.keySet()) {
			...
			String fileType = getSymbolType(symbol);
			if (!folders.containsKey(fileType)) {
				folders.put(fileType, fileType);
			}
		}
		for (String tag : folders.keySet()) {
			writer.writeStartElement("DOMFolderItem", new String[] { "name", tag });
			writer.writeEndElement();
		}
		writer.writeEndElement();

		convertMedia(swf, characterVariables, characterClasses, nonLibraryShapes, backgroundColor, tags, characters, files, datfiles,
				flaVersion, writer);
		convertSymbols(swf, characterVariables, characterClasses, characterScriptPacks, nonLibraryShapes, backgroundColor, tags,
				characters, files, datfiles, flaVersion, writer);
	}

	static String getSymbolType(CharacterTag symbol) {
		...
		if (symbol instanceof ButtonTag) {
			...
			fileType = "Shape";
		} else if (symbol instanceof FontTag) {
			fileType = "Font";
	...
	static boolean convertSymbolName = true;

	private void convertSymbols(SWF swf, Map<Integer, String> characterVariables, Map<Integer, String> characterClasses,
			Map<Integer, ScriptPack> characterScriptPacks, List<Integer> nonLibraryShapes, String backgroundColor, ReadOnlyTagList tags,
			HashMap<Integer, CharacterTag> characters, HashMap<String, byte[]> files, HashMap<String, byte[]> datfiles,
			FLAVersion flaVersion, XFLXmlWriter writer) throws XMLStreamException {
		...
		for (int ch : characters.keySet()) {
			...
			if ((symbol instanceof ShapeTag) || (symbol instanceof DefineSpriteTag) || (symbol instanceof ButtonTag)) {
				...
				symbolStr.writeStartElement("DOMSymbolItem", new String[] { "xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance",
						"xmlns", "http://ns.adobe.com/xfl/2008/", "name", fileType + "/Symbol" + symbol.getCharacterId(), "lastModified",
				...
				if (symbol instanceof ButtonTag) {
					itemIcon = "0";
					String timeLineName = "Symbol" + symbol.getCharacterId();
					if (convertSymbolName) {
						if (symbol.getClassName() != null && !symbol.getClassName().isEmpty()) {
							timeLineName = symbol.getClassName();
						}
					}
					symbolStr.writeStartElement("DOMTimeline", new String[] { "name", timeLineName, "currentFrame", "0" });
								...
								if (rec.placeDepth == i) {
									...
									if (character != null) {
										...
										if ((character instanceof ShapeTag) && (nonLibraryShapes.contains(characterId))) {
											...
											convertShape(characters, matrix, shape.getShapeNum(), shape.getShapes().shapeRecords,
													shape.getShapes().fillStyles, shape.getShapes().lineStyles, false, false,
													recCharWriter, characterClasses);
											...
											convertSymbolInstance(null, matrix, colorTransformAlpha, false, blendMode, filters, true, null,
													null, null, characters.get(rec.characterId), characters, tags, flaVersion,
													recCharWriter, characterClasses);
										}
										...
										if (duration > 0) {
											if (duration > 1) {
												symbolStr.writeStartElement(
														"DOMFrame",
														new String[] { "index", Integer.toString(frame - duration), "duration",
																Integer.toString(duration - 1), "keyMode",
																Integer.toString(KEY_MODE_NORMAL), });
												symbolStr.writeElementValue("elements", "");
												symbolStr.writeEndElement();
											}
											symbolStr.writeStartElement("DOMFrame", new String[] { "index", Integer.toString(frame - 1),
					...
					final ScriptPack spriteScriptPack = characterScriptPacks.containsKey(sprite.spriteId) ? characterScriptPacks
							.get(sprite.spriteId) : null;
					convertTimeline(sprite.spriteId, nonLibraryShapes, backgroundColor, tags, sprite.getTags(), characters, timeLineName,
							flaVersion, files, symbolStr, spriteScriptPack, characterClasses);
					...
					if (shapeWithStyle != null) {
						convertShape(characters, null, shape.getShapeNum(), shapeWithStyle.shapeRecords, shapeWithStyle.fillStyles,
								shapeWithStyle.lineStyles, false, true, symbolStr, characterClasses);
					}
				...
				String symbolFile = fileType + "/Symbol" + symbol.getCharacterId() + ".xml";
	...
	private void convertMedia(SWF swf, Map<Integer, String> characterVariables, Map<Integer, String> characterClasses,
			List<Integer> nonLibraryShapes, String backgroundColor, ReadOnlyTagList tags, HashMap<Integer, CharacterTag> characters,
			HashMap<String, byte[]> files, HashMap<String, byte[]> datfiles, FLAVersion flaVersion, XFLXmlWriter writer)
			throws XMLStreamException {
		boolean hasMedia = false;
		for (int ch : characters.keySet()) {
			CharacterTag symbol = characters.get(ch);
			if (symbol instanceof ImageTag || symbol instanceof SoundStreamHeadTypeTag || symbol instanceof DefineSoundTag
		...
		for (int ch : characters.keySet()) {
			...
			if (symbol instanceof ImageTag) {
				...
				looptags: for (Tag tag : swf.getTags()) {
					if (tag instanceof ShapeTag) {
						...
						if (needed.contains(imageTag.getCharacterId())) {
							...
							for (FILLSTYLE f : fs) {
								if (Arrays.asList(FILLSTYLE.REPEATING_BITMAP, FILLSTYLE.CLIPPED_BITMAP,
										FILLSTYLE.NON_SMOOTHED_REPEATING_BITMAP, FILLSTYLE.NON_SMOOTHED_CLIPPED_BITMAP).contains(
										f.fillStyleType)
										&& f.bitmapId == imageTag.getCharacterId()) {
				...
				if (convertSymbolName) {
					if (symbol.getClassName() != null && !symbol.getClassName().isEmpty()) {
						symbolFile = symbol.getClassName() + "_" + symbolFile;
					}
				}
				// String symbolFile = fileType + "/bitmap" + symbol.getCharacterId()
				// + imageTag.getImageFormat().getExtension();
				String symbolName = symbolFile;
				files.put(symbolFile, imageBytes);
				writer.writeStartElement("DOMBitmapItem",
						new String[] { "name", symbolName, "sourceLastImported", Long.toString(getTimestamp(swf)), "externalFileSize",
				...
				int format = 0;
				int bits = 0;
				if ((soundFormat == SoundFormat.FORMAT_ADPCM) || (soundFormat == SoundFormat.FORMAT_UNCOMPRESSED_LITTLE_ENDIAN)
				...
				// String symbolFile = fileType + "sound" + symbol.getCharacterId() + "." + exportFormat;
				String symbolName = symbolFile;// "sound" + symbol.getCharacterId() + "." + exportFormat;
				files.put(symbolFile, data);
				writer.writeStartElement("DOMSoundItem",
						new String[] { "name", symbolName, "sourceLastImported", Long.toString(getTimestamp(swf)), "externalFileSize",
								Integer.toString(data.length) });
				writer.writeAttribute("href", symbolFile);
				writer.writeAttribute("format", rateMap[soundRate] + "kHz" + " " + (soundSize ? "16bit" : "8bit") + " "
				...
				// String symbolFile = fileType + "/movie" + symbol.getCharacterId() + "." + "flv";
				...
				String symbolName = symbolFile;// "movie" + symbol.getCharacterId() + "." + "flv";
				if (data.length == 0) { // Video has zero length, this probably
					...
					writer.writeEmptyElement("DOMVideoItem", new String[] { "name", symbolName, "sourceExternalFilepath",
							"./LIBRARY/" + symbolFile, "sourceLastImported", Long.toString(ts), "videoDataHRef", datFileName, "channels",
							"0", "isSpecial", "true" });
					...
					files.put(symbolFile, data);
					writer.writeStartElement("DOMVideoItem",
							new String[] { "name", symbolName, "sourceLastImported", Long.toString(getTimestamp(swf)), "externalFileSize",
									Integer.toString(data.length) });
					writer.writeAttribute("href", symbolFile);
	...
	public static void main(String[] argv) {
		String xml = "<DOMSymbolItem xmlns=\"http://ns.adobe.com/xfl/2008/\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" name=\"Sprite/Symbol55\" lastModified=\"1545829041\"><timeline><DOMTimeline name=\"Symbol55\"><layers><DOMLayer name=\"Layer1\" color=\"#9fe08c\" current=\"true\" isSelected=\"true\"> <frames><DOMFrame index=\"0\" keyMode=\"9728\"><elements><DOMShape isFloating=\"true\"> <fills><FillStyle index=\"1\"><SolidColor color=\"#ffffff\"/></FillStyle><FillStyle index=\"2\"><SolidColor color=\"#e9f3f4\"/></FillStyle><FillStyle index=\"3\"><SolidColor color=\"#c4cdcd\"/></FillStyle></fills> <strokes/><edges><Edge fillStyle0=\"3\" edges=\"!0 0! -460 -558[ -670 -560 -786 -378[ -886 -221 -859 -35[ -844 68 -793 157[ -988 148 -1174 193[ -1261 213 -1327 274[ -1367 313 -1338 360[ -1324 383 -1301 400[ -1220 458 -1116 478[ -900 519 -683 486| -486 448[ -451 438 -413 447[ -264 475 -131 410[ -79 481 0 513[ 213 597 432 543| 501 521| 682 449[ 936 532 1195 475[ 1268 460 1325 413[ 1370 374 1337 327[ 1311 288 1267 274[ 1070 215 866 235[ 800 114 666 81[ 515 38 371 102[ 319 126 266 115[ 248 38 170 6[ 91 -30 2 -42| -134 -38[ -109 -232 -211 -392[ -286 -511 -421 -549| -460 -558\"/><Edge fillStyle0=\"3\" fillStyle1=\"1\" edges=\"!-460 -558! -1044 446[ -1160 429 -1265 377[ -1287 367 -1303 347| -1316 325[ -1293 281 -1248 263[ -1178 238 -1104 223[ -926 186 -744 197| -733 178[ -859 -8 -809 -230[ -789 -322 -731 -398[ -677 -468 -593 -497[ -496 -531 -400 -498[ -343 -480 -299 -433[ -149 -274 -172 -53[ -175 -26 -171 3[ 17 -26 180 64[ 203 78 216 100| 233 149[ 300 167 368 136[ 406 119 447 109[ 527 93 612 108[ 707 118 777 182| 807 220| 841 276[ 1063 246 1272 319[ 1291 327 1302 346| 1316 370| 1300 389\"/><Edge fillStyle0=\"3\" fillStyle1=\"2\" edges=\"!1300 389[ 1116 474 911 455| 688 419[ 661 414 635 426[ 569 460 499 485[ 306 551 108 507[ 14 484 -62 423[ -91 398 -110 366| -120 362[ -282 445 -463 421| -575 429[ -809 478 -1044 446\"/><Edge fillStyle0=\"1\" fillStyle1=\"2\" edges=\"!-1044 446[ -859 432 -680 376[ -538 329 -428 240[ -404 287 -356 305[ -260 335 -160 317| 6 290[ 104 271 203 240| 194 267| 188 293[ 186 307 187 321[ 188 344 196 366| 209 388[ 251 406 300 404[ 435 389 561 335[ 784 382 1013 390[ 1156 396 1300 389\"/><Edge fillStyle0=\"2\" fillStyle1=\"1\" edges=\"!1300 389! -378 334| -404 321| -264 336[ -320 361 -378 334\"/><Edge fillStyle0=\"2\" fillStyle1=\"1\" edges=\"!-378 334! -774 433| -520 332[ -615 418 -743 432| -774 433\"/><Edge fillStyle0=\"2\" fillStyle1=\"1\" edges=\"!-774 433! 144 374[ 135 341 155 313[ 143 403 234 420| 381 430[ 266 470 169 408[ 150 395 144 374\"/></edges></DOMShape> </elements></DOMFrame> </frames></DOMLayer> </layers></DOMTimeline></timeline></DOMSymbolItem>";
		System.err.println(prettyFormatXML(xml));
	}

	private static void convertFrame(boolean shapeTween, SoundStreamHeadTypeTag soundStreamHead, StartSoundTag startSound, int frame,
			int duration, String actionScript, String elements, HashMap<String, byte[]> files, XFLXmlWriter writer,
			Map<Integer, String> characterClasses) throws XMLStreamException {
		...
		// String fileType = getSymbolType(startSound);
		XFLXmlWriter soundEnvelopeStr = new XFLXmlWriter();
		if (soundStreamHead != null && startSound == null) {
			// String soundName = "Sound/sound" + soundStreamHead.getCharacterId() + "."
			String soundName = "sound" + soundStreamHead.getCharacterId() + "."
					+ soundStreamHead.getExportFormat().toString().toLowerCase();
			if (convertSymbolName) {
				if (soundStreamHead instanceof CharacterTag) {
					CharacterTag tag = (CharacterTag) soundStreamHead;
					if (tag.getClassName() != null && !tag.getClassName().isEmpty()) {
						soundName = tag.getClassName() + "_" + soundName;
					}
				} else {
					if (characterClasses.containsKey(soundStreamHead.getCharacterId())) {
						soundName = characterClasses.get(soundStreamHead.getCharacterId()) + "_" + soundName;
					}
				}
			}
		...
		if (startSound != null && sound != null) {
			...
			if (convertSymbolName) {
				if (sound.getClassName() != null && !sound.getClassName().isEmpty()) {
					soundName = sound.getClassName() + "_" + soundName;
				}
			}
			// String soundName = "Sound/sound" + sound.soundId + "." +
			...
			if (startSound.soundInfo.hasEnvelope) {
				...
				for (SOUNDENVELOPE env : envelopeRecords) {
					soundEnvelopeStr.writeEmptyElement("SoundEnvelopePoint", new String[] { "mark44", Long.toString(env.pos44), "level0",
							Integer.toString(env.leftLevel), "level1", Integer.toString(env.rightLevel) });
				}

				if (envelopeRecords.length == 1 && envelopeRecords[0].leftLevel == 32768 && envelopeRecords[0].pos44 == 0
						&& envelopeRecords[0].rightLevel == 0) {
					writer.writeAttribute("soundEffect", "left channel");
				} else if (envelopeRecords.length == 1 && envelopeRecords[0].leftLevel == 0 && envelopeRecords[0].pos44 == 0
						&& envelopeRecords[0].rightLevel == 32768) {
					writer.writeAttribute("soundEffect", "right channel");
				} else if (envelopeRecords.length == 2 && envelopeRecords[0].leftLevel == 32768 && envelopeRecords[0].pos44 == 0
						&& envelopeRecords[0].rightLevel == 0 && envelopeRecords[1].leftLevel == 0
						&& envelopeRecords[1].pos44 == sound.soundSampleCount && envelopeRecords[1].rightLevel == 32768) {
					writer.writeAttribute("soundEffect", "fade left to right");
				} else if (envelopeRecords.length == 2 && envelopeRecords[0].leftLevel == 0 && envelopeRecords[0].pos44 == 0
						&& envelopeRecords[0].rightLevel == 32768 && envelopeRecords[1].leftLevel == 32768
						&& envelopeRecords[1].pos44 == sound.soundSampleCount && envelopeRecords[1].rightLevel == 0) {
	...
	private static void convertVideoInstance(String instanceName, MATRIX matrix, DefineVideoStreamTag video, CLIPACTIONS clipActions,
			XFLXmlWriter writer) throws XMLStreamException {

		String videoName = "movie" + video.characterID + ".flv";
		if (convertSymbolName) {
			if (video.getClassName() != null && !video.getClassName().isEmpty()) {
				videoName = video.getClassName() + "_" + videoName;
			}
		}
		writer.writeStartElement("DOMVideoInstance",
				new String[] { "libraryItemName", videoName, "frameRight", Integer.toString((int) (SWF.unitDivisor * video.width)),
	...
	private static void convertFrames(String prevStr, String afterStr, List<Integer> nonLibraryShapes, ReadOnlyTagList tags,
			ReadOnlyTagList timelineTags, HashMap<Integer, CharacterTag> characters, int depth, FLAVersion flaVersion,
			HashMap<String, byte[]> files, XFLXmlWriter writer, Map<Integer, String> characterClasses) throws XMLStreamException {
		...
		for (Tag t : timTags) {
			...
			if (t instanceof ShowFrameTag) {
				...
				if ((character instanceof ShapeTag) && (nonLibraryShapes.contains(characterId) || shapeTweener != null)) {
					ShapeTag shape = (ShapeTag) character;
					convertShape(characters, matrix, shape.getShapeNum(), shape.getShapes().shapeRecords, shape.getShapes().fillStyles,
							shape.getShapes().lineStyles, false, false, elementsWriter, characterClasses);
					shapeTween = false;
					shapeTweener = null;
				} else if (character != null) {
					if (character instanceof MorphShapeTag) {
						MorphShapeTag m = (MorphShapeTag) character;
						convertShape(characters, matrix, 3, m.getStartEdges().shapeRecords, m.getFillStyles().getStartFillStyles(), m
								.getLineStyles().getStartLineStyles(m.getShapeNum()), true, false, elementsWriter, characterClasses);
						shapeTween = true;
					} else {
						shapeTween = false;
						if (character instanceof TextTag) {
							...
							convertSymbolInstance(instanceName, matrix, colorTransForm, cacheAsBitmap, blendMode, filters, isVisible,
									backGroundColor, clipActions, metadata, character, characters, tags, flaVersion, elementsWriter,
									characterClasses);
						}
					}
				}
				...
				if (!elements.equals(lastElements) && frame > 0) {
					convertFrame(lastShapeTween, null, null, frame - duration, duration, "", lastElements, files, writer2, characterClasses);
					duration = 1;
				} else if (frame == 0) {
					duration = 1;
				} else {
		...
		if (!lastElements.isEmpty()) {
			frame++;
			convertFrame(lastShapeTween, null, null, (frame - duration < 0 ? 0 : frame - duration), duration, "", lastElements, files,
					writer2, characterClasses);
		}
	...
	private static void convertFonts(ReadOnlyTagList tags, XFLXmlWriter writer) throws XMLStreamException {
		...
		for (Tag t : tags) {
			if (t instanceof FontTag) {
				...
				writer.writeStartElement("DOMFontItem", new String[] { "name", "Font " + fontId, "font", fontName, "size", "0", "id",
		...
		if (classIndex > -1) {
			...
			try {
				if (constructorBody.convertedItems == null) {
					constructorBody.convert(new ConvertData(), "??", ScriptExportMode.AS, true, constructorMethodIndex, pack.scriptIndex,
							classIndex, abc, null, new ScopeStack(), GraphTextWriter.TRAIT_INSTANCE_INITIALIZER, new NulWriter(),
				...
				/*
				 * It looks like this: CallPropertyAVM2Item ->propertyName == FullMultinameAVM2Item -> resolvedMultinameName (String)
				 * "addFrameScript" ->arguments ->0 IntegerValueAVM2Item ->value (Long) 0 - zero based ->1 GetPropertyAVM2Item ->object
				 * (ThisAVM2Item) ->propertyName (FullMultinameAvm2Item) ->multinameIndex ->resolvedMultinameName (String) "frame1"
				 */
				if (constructorBody.convertedItems != null) {
				...
				for (int frame : frameToTraitMultiname.keySet()) {
					...
					if (multinameToMethodTrait.containsKey(multiName)) {
						...
						frameBody.convert(new ConvertData(), "??", ScriptExportMode.AS, false, methodIndex, pack.scriptIndex, classIndex,
								abc, methodTrait, new ScopeStack(), 0, new NulWriter(), new ArrayList<>(), new ArrayList<>(), true);
	...
	private boolean convertActionScriptLayer(int spriteId, ReadOnlyTagList tags, ReadOnlyTagList timeLineTags, String backgroundColor,
			XFLXmlWriter writer, ScriptPack scriptPack) throws XMLStreamException {
	...
	private boolean convertLabelsLayer(int spriteId, ReadOnlyTagList tags, ReadOnlyTagList timeLineTags, String backgroundColor,
			XFLXmlWriter writer) throws XMLStreamException {
	...
	private void convertSoundLayer(int layerIndex, ReadOnlyTagList timeLineTags, HashMap<String, byte[]> files, XFLXmlWriter writer,
			Map<Integer, String> characterClasses) throws XMLStreamException {
		...
		for (Tag t : timeLineTags) {
			if (t instanceof StartSoundTag) {
				...
				// if (!files.containsKey("Sound/sound" + s.soundId + "." +
				// s.getExportFormat().toString().toLowerCase())) {
				if (!files.containsKey("sound" + s.soundId + "." + s.getExportFormat().toString().toLowerCase())) {
					// Sound was not exported
				...
				// if (!files.containsKey("Sound/sound" + soundStreamHead.getCharacterId() + "."
				if (!files.containsKey("sound" + soundStreamHead.getCharacterId() + "."
						+ soundStreamHead.getExportFormat().toString().toLowerCase())) { // Sound was not exported
		...
		for (int i = 0; i < soundStreamHeads.size(); i++) {
			writer.writeStartElement("DOMLayer",
					new String[] { "name", "Sound Layer " + (soundLayerIndex++), "color", randomOutlineColor() });
			writer.writeStartElement("frames");
			...
			if (startFrame != 0) {
				...
				convertFrame(false, null, null, 0, startFrame, "", "", files, writer, characterClasses);
			}

			convertFrame(false, soundStreamHeads.get(i), null, startFrame, duration, "", "", files, writer, characterClasses);
		...
		for (int i = 0; i < startSounds.size(); i++) {
			writer.writeStartElement("DOMLayer", new String[] { "name", "SoundLayer" + (soundLayerIndex++), "color", randomOutlineColor() });
			writer.writeStartElement("frames");
			...
			convertFrame(false, null, startSounds.get(i), startFrame, duration, "", "", files, writer, characterClasses);
	...
	private void convertTimeline(int spriteId, List<Integer> nonLibraryShapes, String backgroundColor, ReadOnlyTagList tags,
			ReadOnlyTagList timelineTags, HashMap<Integer, CharacterTag> characters, String name, FLAVersion flaVersion,
			HashMap<String, byte[]> files, XFLXmlWriter writer, ScriptPack scriptPack, Map<Integer, String> characterClasses)
			throws XMLStreamException {
		...
		for (int d = layerCount; d >= 1; d--, index++) {
			for (Tag t : timelineTags) {
				if (t instanceof PlaceObjectTypeTag) {
					...
					if (po.getClipDepth() == d) {
						...
						writer.writeStartElement("DOMLayer", new String[] { "name", "Layer" + (index + 1), "color", randomOutlineColor(),
								"layerType", "mask", "locked", "true" });
			...
			layerPrev.writeStartElement("DOMLayer", new String[] { "name", "Layer" + (index + 1), "color", randomOutlineColor() });
			if (d == 1) {
		...
		layerCount++;
		convertSoundLayer(soundLayerIndex, timelineTags, files, writer, characterClasses);
		writer.writeEndElement();
		writer.writeEndElement();
	}
	...
	private static void convertText(String instanceName, TextTag tag, MATRIX m, List<FILTER> filters, CLIPACTIONS clipActions,
			XFLXmlWriter writer) throws XMLStreamException {
			...
			for (int r = 0; r < textRecords.size(); r++) {
				...
				if (font != null) {
					...
					writer.writeStartElement(
							"DOMTextAttrs",
							new String[] { "aliasText", "false", "rotation", "true", "size", Double.toString(twipToPixel(textHeight)),
									"bitmapSize", Integer.toString(textHeight), "letterSpacing",
									doubleToString(twipToPixel(letterSpacings.get(r))), "indent",
									doubleToString(twipToPixel((int) attrs.get("indent"))), "leftMargin",
									doubleToString(twipToPixel(leftMargins.get(r))), "lineSpacing",
									doubleToString(twipToPixel((int) attrs.get("lineSpacing"))), "rightMargin",
									doubleToString(twipToPixel((int) attrs.get("rightMargin"))) });
				...
				if (det.hasFont) {
					...
					if (ft != null) {
						...
						if ((installedFont = FontTag.isFontFamilyInstalled(fontName)) != null) {
							...
							fontFace = new Font(installedFont, (italic ? Font.ITALIC : 0) | (bold ? Font.BOLD : 0)
									| (!italic && !bold ? Font.PLAIN : 0), size < 0 ? 10 : size).getPSName();
						}
	...
	public void convertSWF(AbortRetryIgnoreHandler handler, SWF swf, String swfFileName, String outfile, XFLExportSettings settings,
			String generator, String generatorVerName, String generatorVersion, boolean parallel, FLAVersion flaVersion)
		...
		try {
			domDocument.writeStartElement("DOMDocument", new String[] { "xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance", "xmlns",
					"http://ns.adobe.com/xfl/2008/", "currentTimeline", "1", "xflVersion", flaVersion.xflVersion(), "creatorInfo",
					generator, "platform", "Windows", "versionInfo", "Saved by " + generatorVerName, "majorVersion", generatorVersion,
					"buildNumber", "", "nextSceneIdentifier", "2", "playOptionsPlayLoop", "false", "playOptionsPlayPages", "false",
					"playOptionsPlayFrameActions", "false", "autoSaveHasPrompted", "true", "backgroundColor", backgroundColor, "frameRate",
					Integer.toString((int) swf.frameRate)
			// todo: is the cast to int needed?
					});
			...
			convertLibrary(swf, characterVariables, characterClasses, characterScriptPacks, nonLibraryShapes, backgroundColor,
					swf.getTags(), characters, files, datfiles, flaVersion, domDocument);
			...
			convertTimeline(0, nonLibraryShapes, backgroundColor, swf.getTags(), swf.getTags(), characters, "Scene 1", flaVersion, files,
					domDocument, documentScriptPack, characterClasses);
					...
					if (characters.containsKey(chid)) {
						if (characters.get(chid) instanceof DefineSpriteTag) {
							...
							if (sprite.getTags().isEmpty()) {
								...
								File cdir = new File(outDir.getAbsolutePath() + File.separator + "code" + File.separator + expDir);
		...
		try {
			...
			if (greaterThanCC) {
				...
				publishSettings.writeEndElement();
				publishSettings.writeEndElement();
				publishSettings.writeElementValueRaw("LibraryVersions", Helper.newLine + "      "); // todo: is this
																									// really
																									// needed or an
																									// empty tag is
																									// ok?
				...
				publishSettings.writeElementValue("swc-path", "$(AppConfig)/ActionScript 3.0/libs/11.0/textLayout.swc");
				publishSettings.writeEmptyElement("feature", new String[] { "name", "tlfText", "majorVersion", "2", "minorVersion", "0",
						"build", "232" });
				publishSettings.writeElementValue("rsl-url", "http://fpdownload.adobe.com/pub/swz/tlf/2.0.0.232/textLayout_2.0.0.232.swz");
				publishSettings.writeElementValue("policy-file-url", "http://fpdownload.adobe.com/pub/swz/crossdomain.xml");
				publishSettings.writeElementValue("rsl-url", "textLayout_2.0.0.232.swz");
		...
		if (settings.compressed) {
			...
			writeFile(handler, Utf8Helper.getBytes("PROXY-CS6"), outfile);
		}
		if (useAS3 && settings.exportScript) {
			try {
				...
				swf.exportActionScript(handler, outDir.getAbsolutePath() + file.separator + "code", scriptExportSettings, parallel, null);
			} catch (Exception ex) {
	...
	private static void convertAdjustColorFilter(COLORMATRIXFILTER filter, XFLXmlWriter writer) throws XMLStreamException {
		...
		b = (24872168661075.0
				...
				* a13
				+ 43556251470.0
				* Math.sqrt(216225 * a11 * a11 + 332369 * a12 * a12 - 397828 * a12 * a13 + 281684 * a13 * a13 - 930 * a11
						* (287 * a12 + 178 * a13))
				+ 2384730956550.0
				* a12
				* a41
				...
				* a41
				- 685925220
				* Math.sqrt(216225 * a11 * a11 + 332369 * a12 * a12 - 397828 * a12 * a13 + 281684 * a13 * a13 - 930 * a11
						* (287 * a12 + 178 * a13)) * a41 + 465 * a11
				* (466201717582.0 * a12 + 55756962908.0 * a13 + 764132175 * (-127 + 2 * a41)))
				/ (391687695450.0 * a11 * a11 + 5371575610858.0 * a12 * a12 + 1298089188904.0 * a12 * a13 - 72319604312.0 * a13 * a13 + 1860
						* a11 * (1835439833 * a12 + 219515602 * a13));
		c = (127 * (495225 * a11 + 1661845 * a12 + 167930 * a13 + 478 * Math.sqrt(216225 * a11 * a11 + 332369 * a12 * a12 - 397828 * a12
				* a13 + 281684 * a13 * a13 - 930 * a11 * (287 * a12 + 178 * a13)))) / 717495;
		h = 2 * (Math.atan((-465 * a11 + 287 * a12 + 178 * a13 + Math.sqrt(216225 * a11 * a11 + 332369 * a12 * a12 - 397828 * a12 * a13
		...
		s = (1543 * (-103355550
				...
				* Math.sqrt(216225 * a11 * a11 + 332369 * a12 * a12 - 397828 * a12 * a13 + 281684 * a13 * a13 - 930 * a11
						* (287 * a12 + 178 * a13)) + 465
				* a11
				* (274372 * a12 + 170168 * a13 + 1065 * Math.sqrt(216225 * a11 * a11 + 332369 * a12 * a12 - 397828 * a12 * a13 + 281684
						* a13 * a13 - 930 * a11 * (287 * a12 + 178 * a13)))))
				/ (195843847725.0 * a11 * a11 + 2685787805429.0 * a12 * a12 + 649044594452.0 * a12 * a13 - 36159802156.0 * a13 * a13 + 930
						* a11 * (1835439833 * a12 + 219515602 * a13));

		if (sameDouble(410 * a12, 1543 * a31) && sameDouble(410 * a12, 1543 * a32) && sameDouble(3047 * a12, 1543 * a21)
				&& sameDouble(3047 * a12, 1543 * a23) && sameDouble(a22, a11 + (1504 * a12) / 1543.)
				&& sameDouble((1133 * a12) / 1543. + a33, a11)
				/*
				 * && (b == (195961 * a11 + 439039 * a12 + 1543 * (-127 + 2 * a41)) / (3086 * a11 + 6914 * a12)) && (c == 127 * a11 +
				 * (439039 * a12) / 1543.) && (s == (1543 * (a11 - a12)) / (1543 * a11 + 3457 * a12))
				 */&& !sameDouble(a11, a12) && !sameDouble(1543 * a11 + 3457 * a12, 0)) {
			h = 0;
		}

		writer.writeEmptyElement("AdjustColorFilter",
				new String[] { "brightness", Integer.toString(normBrightness(b)), "contrast", Integer.toString(normContrast(c)),
						"saturation", Integer.toString(normSaturation(s)), "hue", Integer.toString(normHue(h)), });
	}
		...
		public HTMLTextParser(ReadOnlyTagList tags, DefineEditTextTag det) {
			if (det.hasFont) {
				...
				if (ft != null) {
					...
					bold = ft.isBold();
					size = det.fontHeight;
					fontFace = new Font(fontName,
							(italic ? Font.ITALIC : 0) | (bold ? Font.BOLD : 0) | (!italic && !bold ? Font.PLAIN : 0), size < 0 ? 10 : size)
							.getPSName();
				}
			}
			if (det.hasLayout) {

...
public class FunctionActionItem extends ActionItem {
	...
	public FunctionActionItem(GraphSourceItem instruction,
			GraphSourceItem lineStartIns, String functionName,
			List<String> paramNames, Map<Integer, String> regNames,
			List<GraphTargetItem> actions, List<String> constants,
			int regStart, List<VariableActionItem> variables,
			List<FunctionActionItem> innerFunctions) {
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		String n = calculatedFunctionName != null ? calculatedFunctionName
				.toStringNoQuotes(localData) : functionName;
		GraphTextWriter nwriter = writer.cloneNew();
		nwriter.startFunction(n);
		...
		if (calculatedFunctionName != null) {
			...
			if (!IdentifiersDeobfuscation.isValidName(false, fname)) {
				IdentifiersDeobfuscation.appendObfuscatedIdentifier(fname,
						nwriter);
			} else {
				calculatedFunctionName.appendToNoQuotes(nwriter, localData);
			}
			...
			if (!IdentifiersDeobfuscation.isValidName(false, functionName)) {
				IdentifiersDeobfuscation.appendObfuscatedIdentifier(
						functionName, nwriter);
			} else {
				nwriter.append(functionName);
			}
		}
		nwriter.spaceBeforeCallParenthesies(paramNames.size());
		nwriter.append("(");
		...
		for (int p = 0; p < paramNames.size(); p++) {
			...
			if (!IdentifiersDeobfuscation.isValidName(false, pname)) {
				IdentifiersDeobfuscation.appendObfuscatedIdentifier(pname,
						nwriter);
			} else {
				nwriter.append(pname);
			}
		}
	...
	public Object getResult() {
		if (!actions.isEmpty()) {
			if (actions.get(actions.size() - 1) instanceof ReturnActionItem) {
				ReturnActionItem r = (ReturnActionItem) actions.get(actions
						.size() - 1);
	...
	private Set<String> getDefinedVariableNames(
			List<VariableActionItem> variables) {
	...
	private void getDeeplyUsedVariableNames(
			Set<String> topLevelDefinedVariableNames, FunctionActionItem fun,
			Set<String> deeplyUsedVariableNames) {

		Set<String> definedVarNamesInFunc = getDefinedVariableNames(fun.variables);
		for (VariableActionItem v : fun.variables) {
			if (!v.isDefinition()
					&& !definedVarNamesInFunc.contains(v.getVariableName())
					&& topLevelDefinedVariableNames.contains(v
							.getVariableName())) {
		...
		for (FunctionActionItem innerFun : fun.innerFunctions) {
			getDeeplyUsedVariableNames(topLevelDefinedVariableNames, innerFun,
					deeplyUsedVariableNames);
		}
	}
	...
	public List<GraphSourceItem> toSource(SourceGeneratorLocalData localData,
		...
		// If the function parameter or local variable is used in inner
		// function,
		...
		for (FunctionActionItem fun : innerFunctions) {
			getDeeplyUsedVariableNames(topLevelVariableNames, fun,
					deeplyUsedVariableNames);
		}
		...
		if (actions != null && !actions.isEmpty()) {
			...
			for (VariableActionItem v : variables) {
				...
				if (registerNames.contains(varName)) {
					if (stored != null) {
						v.setBoxedValue(new StoreRegisterActionItem(null, null,
								new RegisterNumber(registerNames
										.indexOf(varName), varName), stored,
								false));
					} else {
						v.setBoxedValue(new DirectValueActionItem(
								new RegisterNumber(registerNames
										.indexOf(varName), varName)));
					}
				} else if (v.isDefinition()) {
					v.setBoxedValue(new DefineLocalActionItem(null, null,
							((ActionSourceGenerator) generator)
									.pushConstTargetItem(varName), stored));
				} else if (stored != null) {
					v.setBoxedValue(new SetVariableActionItem(null, null,
							((ActionSourceGenerator) generator)
									.pushConstTargetItem(varName), stored));
				} else {
					v.setBoxedValue(new GetVariableActionItem(null, null,
							((ActionSourceGenerator) generator)
									.pushConstTargetItem(varName)));
				}

			}
			for (int i = 1 /* zero is not preloaded */; i < registerNames
					.size(); i++) {
				localDataCopy.registerVars.put(registerNames.get(i), i);
			}

			ret.addAll(asGenerator.toActionList(asGenerator.generate(
					localDataCopy, actions)));
		...
		int len = Action.actionsToBytes(asGenerator.toActionList(ret), false,
				SWF.DEFAULT_VERSION).length;
		if (!needsFun2 && paramNames.isEmpty()) {
			ret.add(0, new ActionDefineFunction(functionName, paramNames, len,
					SWF.DEFAULT_VERSION));
		} else {
			ret.add(0, new ActionDefineFunction2(functionName,
					preloadParentFlag, preloadRootFlag, suppressParentFlag,
					preloadSuperFlag, suppressArgumentsFlag,
					preloadArgumentsFlag, suppressThisFlag, preloadThisFlag,
					preloadGlobalFlag, regCount, len, SWF.DEFAULT_VERSION,
					paramNames, paramRegs));
		}
	...
	public boolean hasReturnValue() {
		return false; // function actually returns itself, but here is false for
						// generator to not add Pop
	}
}

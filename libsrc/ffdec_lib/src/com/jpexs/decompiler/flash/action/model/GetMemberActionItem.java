...
public class GetMemberActionItem extends ActionItem {
	...
	public GetMemberActionItem(GraphSourceItem instruction,
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		if ((!(memberName instanceof DirectValueActionItem))
				|| (!((DirectValueActionItem) memberName).isString())
				|| (!IdentifiersDeobfuscation.isValidName(false,
						((DirectValueActionItem) memberName)
								.toStringNoQuotes(localData)))) {
			nwriter.append("[");
			memberName.toString(nwriter, localData);
			return nwriter.append("]");
		}
		nwriter.append(".");
		stripQuotes(memberName, localData, nwriter);
	...
	public List<GraphSourceItem> toSource(SourceGeneratorLocalData localData,
		...
		return toSourceMerge(localData, generator, object, memberName,
				new ActionGetMember());
	}

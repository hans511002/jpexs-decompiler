...
public class TryActionItem extends ActionItem implements Block {
	...
	public TryActionItem(List<GraphTargetItem> tryCommands,
			List<GraphTargetItem> catchExceptionNames,
			List<GraphTargetItem> catchExceptionTypes,
			List<List<GraphTargetItem>> catchCommands,
			List<GraphTargetItem> finallyCommands) {
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		nwriter.append("try");
		appendBlock(null, nwriter, localData, tryCommands);
		for (int e = 0; e < catchExceptionNames.size(); e++) {
			nwriter.newLine();
			nwriter.append("catch");
			if (nwriter.getFormatting().spaceBeforeParenthesesCatchParentheses) {
				nwriter.append(" ");
			}
			nwriter.append("(");
			catchExceptionNames.get(e).toStringNoQuotes(nwriter, localData);
			if (catchExceptionTypes.get(e) != null) {
				nwriter.append(":");
				catchExceptionTypes.get(e).toStringNoQuotes(nwriter, localData);
			}
			nwriter.append(")");
		...
		if (catchExceptionNames.isEmpty() || finallyCommands.size() > 0) {
			nwriter.newLine();
			nwriter.append("finally");
			appendBlock(null, nwriter, localData, finallyCommands);
		}
	...
	public List<GraphSourceItem> toSource(SourceGeneratorLocalData localData,
		...
		List<Action> tryCommandsA = asGenerator.toActionList(asGenerator
				.generate(localData, tryCommands));
		List<Action> finallyCommandsA = finallyCommands == null ? null
				: asGenerator.toActionList(asGenerator.generate(localData,
						finallyCommands));
		...
		if (catchCommands != null && !catchCommands.isEmpty()) {
			...
			if (catchExceptionNames.size() == 1
					&& catchExceptionTypes.get(0) == null) { // catch everything
																// without any
																// type
				...
				if (ename instanceof DirectValueActionItem) {
					...
					Logger.getLogger(TryActionItem.class.getName()).log(
							Level.SEVERE, "Invalid catchName, string expected");
				}
				catchInRegisterFlag = false;
				fullCatchBody = GraphTargetItem.toSourceMerge(localData,
						generator, catchCommands.get(0));
			} else {
				...
				if (!allCatched) {
					fullCatchBody.addAll(0, GraphTargetItem.toSourceMerge(
							localData, generator, new ActionPush(
									new RegisterNumber(catchRegister)),
							new ActionThrow()));
				}
				...
				for (int i = catchExceptionNames.size() - 1; i >= 0; i--) {
					...
					if (etype == null) {
						fullCatchBody.addAll(0, GraphTargetItem.toSourceMerge(
								localData, generator,
								new DirectValueActionItem(new RegisterNumber(
										catchRegister)), ename,
								new ActionStackSwap(), new ActionDefineLocal(),
								ebody));
					} else {
						List<GraphSourceItem> ifBody = GraphTargetItem
								.toSourceMerge(localData, generator, ename,
										new ActionStackSwap(),
										new ActionDefineLocal(), ebody);
						fullCatchBody.add(0, new ActionPop());
						int toFinishSize = Action.actionsToBytes(
								asGenerator.toActionList(fullCatchBody), false,
						...
						int ifBodySize = Action.actionsToBytes(ifBodyA, false,
								SWF.DEFAULT_VERSION).length;
						fullCatchBody.addAll(0, ifBody);
						fullCatchBody.addAll(0, GraphTargetItem.toSourceMerge(
								localData, generator, etype, new ActionPush(
										new RegisterNumber(catchRegister)),
								new ActionCastOp(), new ActionPushDuplicate(),
								new ActionPush(Null.INSTANCE),
								new ActionEquals2(), new ActionIf(ifBodySize)));
			...
			catchSize = Action.actionsToBytes(catchCommandsA, false,
					SWF.DEFAULT_VERSION).length;
			tryCommandsA.add(new ActionJump(catchSize));
		}
		int finallySize = 0;
		if (finallyCommandsA != null) {
			finallySize = Action.actionsToBytes(finallyCommandsA, false,
					SWF.DEFAULT_VERSION).length;
		}
		int trySize = Action.actionsToBytes(tryCommandsA, false,
				SWF.DEFAULT_VERSION).length;
		ret.add(new ActionTry(catchInRegisterFlag, finallyCommands != null,
				catchCommands != null, catchName, catchRegister, trySize,
				catchSize, finallySize, SWF.DEFAULT_VERSION));

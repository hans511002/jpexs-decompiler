...
public class InitObjectActionItem extends ActionItem {
	...
	public InitObjectActionItem(GraphSourceItem instruction,
			GraphSourceItem lineStartIns, List<GraphTargetItem> names,
			List<GraphTargetItem> values) {
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		for (int i = values.size() - 1; i >= 0; i--) {
			...
			names.get(i).toStringNoQuotes(nwriter, localData); // AS1/2 do not
																// allow quotes
																// in name here
			nwriter.append(":");
			if (values.get(i) instanceof TernarOpItem) { // Ternar operator
															// contains ":"
				nwriter.append("(");
				values.get(i).toString(nwriter, localData);
				nwriter.append(")");
			} else {
				values.get(i).toString(nwriter, localData);
			}

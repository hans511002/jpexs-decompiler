...
public class CallFunctionActionItem extends ActionItem {
	...
	public CallFunctionActionItem(GraphSourceItem instruction,
			GraphSourceItem lineStartIns, GraphTargetItem functionName,
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		if (functionName instanceof DirectValueActionItem) {
			if (!IdentifiersDeobfuscation.isValidName(false,
					(functionName).toStringNoQuotes(localData))) {
				nwriter.append("eval(");
				functionName.toString(nwriter, localData);
				nwriter.append(")");
			} else {
				functionName.toStringNoQuotes(nwriter, localData);
			}
			// writer.append(IdentifiersDeobfuscation.printIdentifier(false,
			// (functionName).toStringNoQuotes(localData)));
		} else {
			functionName.appendTry(nwriter, localData);
		}
	...
	public List<GraphSourceItem> toSource(SourceGeneratorLocalData localData,
		...
		return toSourceMerge(localData, generator,
				toSourceCall(localData, generator, arguments), functionName,

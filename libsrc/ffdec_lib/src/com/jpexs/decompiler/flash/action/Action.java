...
public abstract class Action implements GraphSourceItem {
	 ...
	 */
	public static final String[] propertyNames = new String[] { "_X", "_Y",
			"_xscale", "_yscale", "_currentframe", "_totalframes", "_alpha",
			"_visible", "_width", "_height", "_rotation", "_target",
			"_framesloaded", "_name", "_droptarget", "_url", "_highquality",
			"_focusrect", "_soundbuftime", "_quality", "_xmouse", "_ymouse" };

	public static final List<String> propertyNamesList = Arrays
	 ...
	 *             When read object is not String
	 */
	protected String lexString(FlasmLexer lex) throws IOException,
	 ...
	 * @throws IOException
	 * @throws ActionParseException
	 *             When read object is not Block startServer
	 */
	protected void lexBlockOpen(FlasmLexer lex) throws IOException,
			ActionParseException {
		ASMParsedSymbol symb = lex.yylex();
	 ...
	 * @throws IOException
	 * @throws ActionParseException
	 *             When read object is not Identifier
	 */
	protected String lexIdentifier(FlasmLexer lex) throws IOException,
			ActionParseException {
		ASMParsedSymbol symb = lex.yylex();
	 ...
	 * @throws IOException
	 * @throws ActionParseException
	 *             When read object is not long value
	 */
	protected long lexLong(FlasmLexer lex) throws IOException,
			ActionParseException {
		ASMParsedSymbol symb = lex.yylex();
	 ...
	 * @throws IOException
	 * @throws ActionParseException
	 *             When read object is not boolean value
	 */
	protected boolean lexBoolean(FlasmLexer lex) throws IOException,
			ActionParseException {
		ASMParsedSymbol symb = lex.yylex();
	 ...
	 *            List of actions
	 * @param addZero
	 *            Whether or not to add 0 UI8 value to the end
	 * @param version
	 *            SWF version
	 * @return Array of bytes
	 */
	public static byte[] actionsToBytes(List<Action> list, boolean addZero,
			int version) {
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		...
		if (addZero
				&& (lastAction == null || !(lastAction instanceof ActionEnd))) {
			baos.write(0);
		}
		return baos.toByteArray();
	}

	public static ByteArrayRange actionsToByteArrayRange(List<Action> list,
			boolean addZero, int version) {
		byte[] bytes = Action.actionsToBytes(list, addZero, version);
		return new ByteArrayRange(bytes);
	}

	public static void setConstantPools(ASMSource src,
			List<List<String>> constantPools, boolean tryInline)
				...
				if (action instanceof ActionConstantPool) {
					...
					if (size > 0xffff && tryInline) {
						for (int i = 0; i < constantPool.size(); i++) {
							int refCount = actions
									.getConstantPoolIndexReferenceCount(i);
							if (refCount == 1) {
								actions.inlineConstantPoolString(i,
										constantPool.get(i));
								constantPool.set(i, "");
							}
						}
	 ...
	 *            List of actions
	 * @param baseAddress
	 *            Address of first action in the list
	 */
	...
	private static void informListeners(List<DisassemblyListener> listeners,
			int pos, int count) {
		DisassemblyListener[] listenersArray = listeners
				.toArray(new DisassemblyListener[listeners.size()]);
	 ...
	 */
	public static GraphTextWriter actionsToString(
			List<DisassemblyListener> listeners, long address, ActionList list,
			int version, ScriptExportMode exportMode, GraphTextWriter writer) {
		if (exportMode == ScriptExportMode.CONSTANTS) {
			return constantPoolActionsToString(listeners, address, list,
					version, exportMode, writer);
		}
		...
		/*
		 * List<ConstantPool> cps = SWFInputStream.getConstantPool(new
		 * ArrayList<DisassemblyListener>(), new ActionGraphSource(list,
		 * version, new HashMap<Integer, String>(), new HashMap<String,
		 * GraphTargetItem>(), new HashMap<String, GraphTargetItem>()), 0,
		 * version, path); if (!cps.isEmpty()) { setConstantPool(list,
		 * cps.get(cps.size() - 1)); }
		 */
		...
		for (int pos = 0; pos < list.size(); pos++) {
			...
			if (exportMode == ScriptExportMode.PCODE_HEX) {
				...
				if (Configuration.showFileOffsetInPcodeHex.get()) {
					nwriter.appendNoHilight("@");
					nwriter.appendNoHilight(Helper.formatHex(fileOffset, 8));
					nwriter.appendNoHilight(" ");
				}

				byte[] bytes = a.getBytes(version);
				nwriter.appendNoHilight(Helper.bytesToHexString(bytes));

				if (Configuration.showOriginalBytesInPcodeHex.get()) {
					if (fileData != null && fileOffset != -1
							&& fileData.length > fileOffset + bytes.length - 1) {
						boolean same = true;
						...
						if (!same) {
							...
							for (int i = 0; i < bytes.length; i++) {
								...
								nwriter.appendNoHilight(Helper
										.byteToHex(fileData[(int) (fileOffset + i)]));
							}
			...
			if (containers.containsKey(offset)) {
				for (int i = 0; i < containers.get(offset).size(); i++) {
					...
					GraphSourceItemContainer cnt = containers.get(offset)
							.get(i);
					int cntPos = containersPos.get(cnt);
					nwriter.appendNoHilight(cnt.getASMSourceBetween(cntPos));
					cntPos++;
			...
			if (Configuration.showAllAddresses.get()
					|| importantOffsets.contains(offset)) {
				if (lastPush) {
					nwriter.newLine();
					lastPush = false;
				}
				nwriter.appendNoHilight("loc");
				nwriter.appendNoHilight(Helper.formatAddress(offset));
				nwriter.appendNoHilight(":");
			}
			...
			if (a.isIgnored()) {
				...
				if (!(a instanceof ActionEnd)) {
					...
					for (int i = 0; i < len; i++) {
						nwriter.appendNoHilight("Nop").newLine();
					}
				}
			} else {
				// if (!(a instanceof ActionNop)) {
				String add = "";
				// honfika: commented out the following lines, because it makes
				// no sense
				/*
				 * if (a instanceof ActionIf) { add = " change: " + ((ActionIf)
				 * a).getJumpOffset(); } if (a instanceof ActionJump) { add =
				 * " change: " + ((ActionJump) a).getJumpOffset(); } add =
				 * "; ofs" + Helper.formatAddress(offset) + add; add = "";
				 */
				if ((a instanceof ActionPush) && lastPush) {
					nwriter.appendNoHilight(" ");
					((ActionPush) a).paramsToStringReplaced(list,
							importantOffsets, exportMode, nwriter);
				} else {
					if (lastPush) {
						nwriter.newLine();
						// lastPush = false;
					...
					nwriter.append("", offset, a.getFileOffset());
					...
					if (fixBranch > -1) {
						nwriter.appendNoHilight("FFDec_DeobfuscatePop");
						if (fixBranch == 0) { // jump
							nwriter.newLine();
							nwriter.appendNoHilight("Jump loc");
							nwriter.appendNoHilight(Helper
									.formatAddress(((ActionIf) a)
											.getTargetAddress()));
						} else {
							// nojump, ignore
						}
					} else {
						a.getASMSourceReplaced(list, importantOffsets,
								exportMode, nwriter);
					}
					nwriter.appendNoHilight(a.isIgnored() ? "; ignored" : "");
					nwriter.appendNoHilight(add);
	...
	public static GraphTextWriter constantPoolActionsToString(
			List<DisassemblyListener> listeners, long address, ActionList list,
			int version, ScriptExportMode exportMode, GraphTextWriter writer) {
		int poolIdx = 0;
		GraphTextWriter nwriter = writer.cloneNew();
		nwriter.appendNoHilight(Helper.constants).newLine();
		for (Action a : list) {
			if (a instanceof ActionConstantPool) {
				...
				for (String c : cPool.constantPool) {
					nwriter.appendNoHilight(constIdx);
					nwriter.appendNoHilight("|");
					nwriter.appendNoHilight(Helper.escapeString(c));
					nwriter.newLine();
					constIdx++;
				}
	 ...
	 * @param container
	 * @param knownAddreses
	 *            List of important offsets to mark as labels
	 * @param exportMode
	 *            PCode or hex?
	 * @return String of P-code source
	 */
	public String getASMSource(ActionList container, Set<Long> knownAddreses,
	...
	public void translate(boolean insideDoInitAction,
			...
			List<GraphTargetItem> output, HashMap<Integer, String> regNames,
			HashMap<String, GraphTargetItem> variables,
			HashMap<String, GraphTargetItem> functions, int staticOperation,
	...
	protected long popLong(TranslateStack stack) {
		...
		if (item instanceof DirectValueActionItem) {
			return (long) (double) EcmaScript
					.toNumberAs2(((DirectValueActionItem) item).value);
		}
	...
	public static long ip2adr(List<Action> actions, int ip) {
		/*
		 * List<Action> actions=new ArrayList<Action>(); for(GraphSourceItem
		 * s:sources){ if(s instanceof Action){ actions.add((Action)s); } }
		 */
		if (ip >= actions.size()) {
			if (actions.isEmpty()) {
				return 0;
			}
			return actions.get(actions.size() - 1).getAddress()
					+ actions.get(actions.size() - 1).getTotalActionLength();
		}
		if (ip == -1) {
			return 0;
		}
	...
	public static int adr2ip(List<Action> actions, long addr) {
		...
		if (actions.size() > 0) {
			long outpos = actions.get(actions.size() - 1).getAddress()
					+ actions.get(actions.size() - 1).getTotalActionLength();
			if (addr == outpos) {
				return actions.size();
			}
	...
	public static List<GraphTargetItem> actionsToTree(
			boolean insideDoInitAction, List<Action> actions, int version,
			int staticOperation, String path) throws InterruptedException {
		return actionsToTree(insideDoInitAction, new HashMap<>(),
				new HashMap<>(), new HashMap<>(), actions, version,
				staticOperation, path);
	}
	 ...
	 * @throws java.lang.InterruptedException
	 */
	public static GraphTextWriter actionsToSource(final ASMSource asm,
			final List<Action> actions, final String path,
			GraphTextWriter writer) throws InterruptedException {
		GraphTextWriter nwriter = writer.cloneNew();
		nwriter.suspendMeasure();
		...
		try {
			tree = CancellableWorker.call(
					new Callable<List<GraphTargetItem>>() {
						@Override
						public List<GraphTargetItem> call() throws Exception {
							int staticOperation = Graph.SOP_USE_STATIC;
							// (Boolean)
							// Configuration.getConfig("autoDeobfuscate", true)
							// ? Graph.SOP_SKIP_STATIC : Graph.SOP_USE_STATIC;
							boolean insideDoInitAction = (asm instanceof DoInitActionTag);
							List<GraphTargetItem> tree = actionsToTree(
									insideDoInitAction, new HashMap<>(),
									new HashMap<>(), new HashMap<>(), actions,
									version, staticOperation, path);
							SWFDecompilerPlugin
									.fireActionTreeCreated(tree, swf);
							if (Configuration.autoDeobfuscate.get()) {
								new ActionDeobfuscator().actionTreeCreated(
										tree, swf);
							}

							Graph.graphToString(tree, new NulWriter(),
									new LocalData());
							return tree;
						}
			...
			if (convertException instanceof TimeoutException) {
				logger.log(Level.SEVERE, "Decompilation timeout in: " + path,
						convertException);
			} else {
				logger.log(Level.SEVERE, "Decompilation error in: " + path,
						convertException);
			}
		...
		if (asm != null) {
			asm.getActionSourcePrefix(nwriter);
		}
		if (convertException == null) {
			Graph.graphToString(tree, nwriter, new LocalData());
		} else if (convertException instanceof TimeoutException) {
			Helper.appendTimeoutCommentAs2(nwriter, timeout, actions.size());
		} else {
			Helper.appendErrorComment(nwriter, convertException);
		}
		if (asm != null) {
			asm.getActionSourceSuffix(nwriter);
		}
		writer.marge(nwriter);
		return writer;
	}
	 ...
	 */
	public static List<GraphTargetItem> actionsToTree(
			boolean insideDoInitAction, HashMap<Integer, String> regNames,
			HashMap<String, GraphTargetItem> variables,
			HashMap<String, GraphTargetItem> functions, List<Action> actions,
			int version, int staticOperation, String path)
			throws InterruptedException {
		return ActionGraph.translateViaGraph(insideDoInitAction, regNames,
				variables, functions, actions, version, staticOperation, path);
	}

	@Override
	public void translate(BaseLocalData localData, TranslateStack stack,
			List<GraphTargetItem> output, int staticOperation, String path)
			throws InterruptedException {
		ActionLocalData aLocalData = (ActionLocalData) localData;
		/*
		 * int expectedSize = stack.size() - getStackPopCount(localData, stack);
		 * if (expectedSize < 0) { expectedSize = 0; } expectedSize +=
		 * getStackPushCount(localData, stack);
		 */

		translate(aLocalData.insideDoInitAction, aLocalData.lineStartAction,
				stack, output, aLocalData.regNames, aLocalData.variables,
				aLocalData.functions, staticOperation, path);
		/*
		 * if (stack.size() != expectedSize && !(this instanceof
		 * ActionPushDuplicate)) { throw new
		 * Error("HONFIKA stack size mismatch"); }
		 */
	}
	...
	public static List<GraphTargetItem> actionsPartToTree(
			boolean insideDoInitAction, Reference<GraphSourceItem> fi,
			HashMap<Integer, String> registerNames,
			HashMap<String, GraphTargetItem> variables,
			HashMap<String, GraphTargetItem> functions, TranslateStack stack,
			List<Action> actions, int start, int end, int version,
			int staticOperation, String path) throws InterruptedException {
		if (start < actions.size() && (end > 0) && (start > 0)) {
			logger.log(
					...
					new Object[] {
							...
							actions.size() > 0 ? (" ("
									+ actions.get(start).toString()
									+ " - "
									+ actions.get(end == actions.size() ? end - 1
											: end) + ")")
									: "" });
		}
		ActionLocalData localData = new ActionLocalData(insideDoInitAction,
				registerNames, variables, functions);
		...
		loopip: while (ip <= end) {
			...
			// FunctionActionItem after DefineFunction(/2) are left on the
			// stack. For linestart offsets we consider this kind of stack
			// empty.
			...
			if (action instanceof GraphSourceItemContainer) {
				...
				// List<GraphTargetItem> out=actionsPartToTree(new
				// HashMap<Integer, String>(), new HashMap<String,
				// GraphTargetItem>(),new HashMap<String, GraphTargetItem>(),
				// new TranslateStack(), src, ip+1,endip-1 , version);
				long endAddr = action.getAddress() + cnt.getHeaderSize();
				String cntName = cnt.getName();
				List<List<GraphTargetItem>> outs = new ArrayList<>();
				HashMap<String, GraphTargetItem> variables2 = Helper
						.deepCopy(variables);
				if (cnt instanceof ActionDefineFunction
						|| cnt instanceof ActionDefineFunction2) {
					for (int r = 0; r < 256; r++) {
				...
				for (long size : cnt.getContainerSizes()) {
					...
					try {
						...
						if (action instanceof ActionWith
								|| action instanceof ActionTry) {
							for (Map.Entry<Integer, String> e : registerNames
									.entrySet()) {
						...
						out = ActionGraph.translateViaGraph(insideDoInitAction,
								regNames, variables2, functions,
								actions.subList(adr2ip(actions, endAddr),
										adr2ip(actions, endAddr + size)),
								version, staticOperation,
								path + (cntName == null ? "" : "/" + cntName));
					} catch (OutOfMemoryError | TranslateException
							| StackOverflowError ex) {
						logger.log(Level.SEVERE, "Decompilation error in: "
								+ path, ex);
						...
						out.add(new CommentItem(
								new String[] {
										...
										" * "
												+ AppResources
														.translate("decompilationError"),
										" * "
												+ AppResources
														.translate("decompilationError.obfuscated"),
										Helper.decompilationErrorAdd == null ? null
												: " * "
														+ Helper.decompilationErrorAdd,
										" * "
												+ AppResources
														.translate("decompilationError.errorType")
												+ ": "
				...
				((GraphSourceItemContainer) action).translateContainer(outs,
						action, stack, output, registerNames, variables,
						functions);
			...
			if ((action instanceof ActionPush)
					&& (((ActionPush) action).values.size() == 1)
					&& (((ActionPush) action).values.get(0) == Null.INSTANCE)) {
				if (ip + 3 <= end) {
					if ((actions.get(ip + 1) instanceof ActionEquals)
							|| (actions.get(ip + 1) instanceof ActionEquals2)) {
						if (actions.get(ip + 2) instanceof ActionNot) {
							if (actions.get(ip + 3) instanceof ActionIf) {
								ActionIf aif = (ActionIf) actions.get(ip + 3);
								if (adr2ip(actions, ip2adr(actions, ip + 3) + 5
			...
			if (action instanceof ActionStore) {
				...
				store.setStore(actions.subList(ip + 1,
						ip + 1 + store.getStoreSize()));
		...
		if (((GetMemberActionItem) t).object instanceof GetVariableActionItem) {
			...
			if (v.name instanceof DirectValueActionItem) {
				if (((DirectValueActionItem) v.name).value instanceof String) {
					if (((DirectValueActionItem) v.name).value
							.equals("_global")) {
						GetVariableActionItem gvt = new GetVariableActionItem(
								null, null,
								((GetMemberActionItem) t).memberName);
						if (lastMember == null) {
							return gvt;
						} else {
	...
	public static void setConstantPool(List<? extends GraphSourceItem> actions,
			ConstantPool cpool) {
		for (GraphSourceItem a : actions) {
			...
			if (a instanceof ActionDefineFunction) {
				if (cpool != null) {
					...
					// a).setConstantPool(cpool.constants,actions);
				}
			}
	...
	public GraphTextWriter getASMSourceReplaced(ActionList container,
			Set<Long> knownAddreses, ScriptExportMode exportMode,
			GraphTextWriter writer) {
		GraphTextWriter nwriter = writer.cloneNew();
		nwriter.appendNoHilight(getASMSource(container, knownAddreses,
				exportMode));
		writer.marge(nwriter);
		return writer;
	}
	...
	public static GraphTargetItem gettoset(GraphTargetItem get,
			GraphTargetItem value, List<VariableActionItem> variables) {
		...
		if (ret instanceof GetVariableActionItem) {
			...
			ret = new SetMemberActionItem(null, null, mem.object,
					mem.memberName, value);
		} else if ((ret instanceof DirectValueActionItem)
				&& ((DirectValueActionItem) ret).value instanceof RegisterNumber) {
			ret = new StoreRegisterActionItem(null, null,
					(RegisterNumber) ((DirectValueActionItem) ret).value,
					value, false);
		} else if (ret instanceof GetPropertyActionItem) {
			GetPropertyActionItem gp = (GetPropertyActionItem) ret;
			ret = new SetPropertyActionItem(null, null, gp.target,
					gp.propertyIndex, value);
		}
		if (boxed) {
			GraphTargetItem b = ret;
			ret = new VariableActionItem(
					((VariableActionItem) get).getVariableName(), value,
					((VariableActionItem) get).isDefinition());

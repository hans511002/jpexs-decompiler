...
public class ClassActionItem extends ActionItem implements Block {
	...
	public ClassActionItem(GraphTargetItem className,
			GraphTargetItem extendsOp, List<GraphTargetItem> implementsOp,
			List<MyEntry<GraphTargetItem, GraphTargetItem>> traits,
			List<Boolean> traitsStatic) {
	...
	private void detectUnitializedVars(GraphTargetItem item,
	...
	private void detectUnitializedVars(List<GraphTargetItem> items,
			List<GraphTargetItem> ret) {
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		nwriter.startClass(className.toStringNoQuotes(localData));
		nwriter.append("class ");
		className.toStringNoQuotes(nwriter, localData);
		if (extendsOp != null) {
			nwriter.append(" extends ");
			extendsOp.toStringNoQuotes(nwriter, localData);
		}
		if (!implementsOp.isEmpty()) {
			nwriter.append(" implements ");
			boolean first = true;
			for (GraphTargetItem t : implementsOp) {
				...
				first = false;
				Action.getWithoutGlobal(t).toString(nwriter, localData);
			}
		}
		nwriter.startBlock();

		/*
		 * if (constructor != null) { constructor.toString(writer,
		 * localData).newLine(); }
		 */
		for (int pass = 1; pass <= 2; pass++) {
			looptraits: for (int i = 0; i < traits.size(); i++) {
				MyEntry<GraphTargetItem, GraphTargetItem> item = traits.get(i);

				switch (pass) {
				// pass 1: add variables
				case 1:
					if (item.getValue() instanceof FunctionActionItem) { // ignore
																			// methods
						continue looptraits;
					}
					break;
				...
				if (traitsStatic.get(i)) {
					nwriter.append("static ");
				}
				if (item.getValue() instanceof FunctionActionItem) {
					item.getValue().toString(nwriter, localData).newLine();
				} else {
					nwriter.append("var ");
					item.getKey().toStringNoQuotes(nwriter, localData);
					nwriter.append(" = ");
					item.getValue().toString(nwriter, localData);
	...
	public List<GraphSourceItem> toSource(SourceGeneratorLocalData localData,
		...
		ret.addAll(asGenerator.generateTraits(localData2, false, className,
				extendsOp, implementsOp, traits, traitsStatic));
		return ret;
	}

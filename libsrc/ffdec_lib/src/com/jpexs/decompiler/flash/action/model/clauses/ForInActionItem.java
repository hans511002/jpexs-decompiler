...
public class ForInActionItem extends LoopActionItem implements Block {
	...
	public ForInActionItem(GraphSourceItem instruction,
			GraphSourceItem lineStartIns, Loop loop,
			GraphTargetItem variableName, GraphTargetItem enumVariable,
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		nwriter.append("(");
		if ((variableName instanceof DirectValueActionItem)
				&& (((DirectValueActionItem) variableName).value instanceof RegisterNumber)) {
			nwriter.append("var ");
		}
		stripQuotes(variableName, localData, nwriter);
		nwriter.append(" in ");
		enumVariable.toString(nwriter, localData);
	...
	public List<GraphSourceItem> toSource(SourceGeneratorLocalData localData,
		...
		HashMap<String, Integer> registerVars = asGenerator
				.getRegisterVars(localData);
		...
		loopBody.addAll(asGenerator.toActionList(variableName
		...
		loopBody.addAll(asGenerator.toActionList(asGenerator.generate(
				localData, commands)));
		asGenerator.setForInLevel(localData, oldForIn);
		ActionJump forinJmpBack = new ActionJump(0);
		loopBody.add(forinJmpBack);
		int bodyLen = Action.actionsToBytes(loopBody, false,
				SWF.DEFAULT_VERSION).length;
		int exprLen = Action.actionsToBytes(loopExpr, false,
				SWF.DEFAULT_VERSION).length;

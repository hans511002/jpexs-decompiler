...
public class ActionList extends ArrayList<Action> {
	...
	public Iterator<Action> getReferencesFor(final Action target) {
		return new Iterator<Action>() {
			private final Iterator<Action> iterator = ActionList.this
					.iterator();
	...
	private int[] getUnreachableActionsMap(int jumpIndex, int jumpTargetIndex) {
		...
		// one item for each action. 1 means reachable, 2 means reachable and
		// processed
			...
			for (int i = 0; i < size; i++) {
				...
				if (isReachable[i] == 1) {
					...
					if (action instanceof ActionJump) {
						...
						long ref = action.getAddress()
								+ action.getTotalActionLength();
						for (Long containerSize : container.getContainerSizes()) {
							ref += containerSize;
							int targetIndex = getIndexByAddress(ref);
							if (targetIndex != -1
									&& isReachable[targetIndex] == 0) {
								isReachable[targetIndex] = 1;
							}
						}
					}
			...
			if (action instanceof ActionPush && action2 instanceof ActionPush) {
				if (!getReferencesFor(action2).hasNext()) {
					...
					if (!(push.constantPool != null
							&& push2.constantPool != null && push.constantPool != push2.constantPool)) {
						ActionPush newPush = new ActionPush(0);
						newPush.constantPool = push.constantPool == null ? push2.constantPool
								: push.constantPool;
	...
	public void saveToFile(String fileName) {
		...
		try (FileTextWriter writer = new FileTextWriter(
				Configuration.getCodeFormatting(), new FileOutputStream(file))) {
			Action.actionsToString(new ArrayList<>(), 0, this,
					SWF.DEFAULT_VERSION, ScriptExportMode.PCODE, writer);
		} catch (IOException ex) {
			Logger.getLogger(ActionList.class.getName()).log(Level.SEVERE,
					null, ex);
		}
	}
	...
	public String toString() {
		...
		actionsToString(new ArrayList<>(), 0, this, SWF.DEFAULT_VERSION,

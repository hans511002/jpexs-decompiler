...
public class ActionPush extends Action {
	...
	public ActionPush(int actionLength, SWFInputStream sis, int version)
			throws IOException {
	...
	public ActionPush(FlasmLexer lexer, List<String> constantPool)
			throws IOException, ActionParseException {
		super(0x96, 0);
		...
		loop: while (true) {
			...
			case ASMParsedSymbol.TYPE_STRING:
				...
				if (constantPool.contains((String) symb.value)) {
					values.add(new ConstantIndex(constantPool
							.indexOf(symb.value)));
				} else {
			...
			case ASMParsedSymbol.TYPE_EOF:
				if (count == 0) {
					throw new ActionParseException("Arguments expected",
							lexer.yyline());
				} else {
					break loop;
				}
	...
	public GraphTextWriter paramsToStringReplaced(
			List<? extends GraphSourceItem> container, Set<Long> knownAddreses,
	...
	public String toStringNoQ(int i) {
		...
		if (value instanceof ConstantIndex) {
			ret = ((ConstantIndex) value).toStringNoQ(constantPool,
					Configuration.resolveConstants.get());
	...
	public GraphTextWriter toString(GraphTextWriter writer) {
		...
		nwriter.appendNoHilight("Push ");
		paramsToString(nwriter);
	...
	public boolean execute(LocalDataArea lda) {
		for (Object value : values) {
			if (value instanceof ConstantIndex) {
				...
				List<String> cPool = lda.constantPool != null ? lda.constantPool
						: constantPool;
		...
		for (Object o : values) {
			if (o instanceof ConstantIndex) {
				if ((constantPool == null)
						|| (((ConstantIndex) o).index >= constantPool.size())) {
					stack.push(new UnresolvedConstantActionItem(
							((ConstantIndex) o).index));
					continue;
				} else {
			...
			/*
			 * if (o instanceof RegisterNumber) { if
			 * (regNames.containsKey(((RegisterNumber) o).number)) {
			 * ((RegisterNumber) o).name = regNames.get(((RegisterNumber)
			 * o).number); } else if (output.size() >= 2) { //chained
			 * assignments:, ignore for class prototype assignment
			 * GraphTargetItem last = output.get(output.size() - 1);
			 * GraphTargetItem prev = output.get(output.size() - 2); if (last
			 * instanceof SetTypeActionItem) { if (prev instanceof
			 * StoreRegisterActionItem) { StoreRegisterActionItem str =
			 * (StoreRegisterActionItem) prev; if (str.register.number ==
			 * ((RegisterNumber) o).number) { SetTypeActionItem stt =
			 * (SetTypeActionItem) last; stt.setTempRegister(((RegisterNumber)
			 * o).number); if ((stt.getValue() instanceof IncrementActionItem)
			 * && (((IncrementActionItem)
			 * stt.getValue()).object.equals(stt.getObject()))) { stack.push(new
			 * PreIncrementActionItem(this, lineStartAction, stt.getObject()));
			 * } else if ((stt.getValue() instanceof DecrementActionItem) &&
			 * (((DecrementActionItem)
			 * stt.getValue()).object.equals(stt.getObject()))) { stack.push(new
			 * PreDecrementActionItem(this, lineStartAction, stt.getObject()));
			 * } else { //stack.push(last); continue; }
			 * output.remove(output.size() - 1); output.remove(output.size() -
			 * 1); pos++; continue; } } } } }
			 */
			if (o instanceof Boolean) {
				...
				DirectValueActionItem dvt = new DirectValueActionItem(this,
						lineStartAction, pos, o, constantPool);

				if (o instanceof RegisterNumber) {// TemporaryRegister
					dvt.computedRegValue = variables.get("__register"
							+ ((RegisterNumber) o).number);
					if (regNames.containsKey(((RegisterNumber) o).number)) {
						((RegisterNumber) o).name = regNames
								.get(((RegisterNumber) o).number);
					}
				}
				if (dvt.computedRegValue instanceof TemporaryRegister) {
					stack.push(new TemporaryRegister(
							((RegisterNumber) o).number,
							((TemporaryRegister) dvt.computedRegValue).value));
				} else {
					stack.push(dvt);
				}
			}

...
public class ActionScriptSearch {
	...
	public List<ActionSearchResult> searchAs2(SWF swf, final String txt,
			boolean ignoreCase, boolean regexp, boolean pcode,
			ScriptSearchListener listener) {
		if (txt != null && !txt.isEmpty()) {
			Map<String, ASMSource> asms = swf.getASMs(false);
			final List<ActionSearchResult> found = new ArrayList<>();
			Pattern pat = regexp ? Pattern
					.compile(
							txt,
				...
				for (Map.Entry<String, ASMSource> item : asms.entrySet()) {
					...
					if (pcode) {
						...
						if (pat.matcher(text).find()) {
							found.add(new ActionSearchResult(asm, pcode, item
									.getKey()));
						}
					} else {
						int fpos = pos;
						Future<HighlightedText> text = SWF
								.getCachedFuture(
										asm,
										null,
										new ScriptDecompiledListener<HighlightedText>() {
											@Override
											public void onStart() {
												if (listener != null) {
													listener.onDecompile(fpos,
															asms.size(),
															item.getKey());
												}
											}
											...
											public void onComplete(
												...
												if (listener != null) {
													listener.onSearch(fpos,
															asms.size(),
															item.getKey());
												}

												if (pat.matcher(result.text)
														.find()) {
													ActionSearchResult searchResult = new ActionSearchResult(
															asm, pcode, item
																	.getKey());
	...
	public List<ABCSearchResult> searchAs3(final SWF swf, final String txt,
			boolean ignoreCase, boolean regexp, boolean pcode,
			ScriptSearchListener listener) {
		// todo: pcode seach
		if (txt != null && !txt.isEmpty()) {
			...
			final Pattern pat = regexp ? Pattern
					.compile(
							txt,
							ignoreCase ? (Pattern.CASE_INSENSITIVE | Pattern.UNICODE_CASE)
									: 0)
			...
			List<Future<HighlightedText>> futures = new ArrayList<>();
			try {
				loop: for (final ScriptPack pack : allpacks) {
					pos++;
					if (!pack.isSimple
							&& Configuration.ignoreCLikePackages.get()) {
						continue;
					}
					if (Configuration._ignoreAdditionalFlexClasses.get()) {
						String fullName = pack.getClassPath().packageStr.add(
								pack.getClassPath().className,
								pack.getClassPath().namespaceSuffix)
								.toRawString();
					...
					if (pcode) {
						if (listener != null) {
							listener.onSearch(pos, allpacks.size(), pack
						...
						for (MethodId methodInfo : methodInfos) {
							int bodyIndex = abc.findBodyIndex(methodInfo
									.getMethodIndex());
							if (bodyIndex != -1) {
								...
								abc.bodies
										...
										.toASMSource(
												abc.constants,
												abc.method_info
														.get(body.method_info),
												body, ScriptExportMode.PCODE,
												writer);
								String text = writer.toText();
								if (pat.matcher(text).find()) {
									ABCSearchResult searchResult = new ABCSearchResult(
											pack, methodInfo.getClassIndex(),
											methodInfo.getTraitId());
									found.add(searchResult);
								}
							}
						}
				...
				for (Future<HighlightedText> future : futures) {
					try {
						...
						Logger.getLogger(ActionScriptSearch.class.getName())

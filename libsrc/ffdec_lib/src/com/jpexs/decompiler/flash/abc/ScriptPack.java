...
public class ScriptPack extends AS3ClassTreeItem {
	...
	private void appendTo(GraphTextWriter writer, List<Trait> traits,
		...
		if (bodyIndex != -1
			...
			nwriter.startTrait(GraphTextWriter.TRAIT_SCRIPT_INITIALIZER);
			nwriter.startMethod(script_init);
	...
	public void toSource(GraphTextWriter writer, final List<Trait> traits,
		...
		try {
			...
			Helper.appendTimeoutCommentAs3(nwriter, timeout, 0);
			writer.marge(nwriter);
			return;
			...
			for (int i = 0; i < txt.length(); i++) {
				blk: {
					...
					if (instr != null) {
						...
						// int origPos =
						// bodyLineToPos.get(bodyIndex).containsKey(line) ?
						// bodyLineToPos.get(bodyIndex).get(line) : -1;

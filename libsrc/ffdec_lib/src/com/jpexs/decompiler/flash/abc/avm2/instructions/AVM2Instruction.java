...
public class AVM2Instruction implements Cloneable, GraphSourceItem {
	...
	public AVM2Instruction(long address, InstructionDefinition definition,
			int[] operands) {
		this.definition = definition;
		this.operands = operands != null && operands.length > 0 ? operands
				: null;
	...
	public int getBytesLength() {
		...
		for (int i = 0; i < definition.operands.length; i++) {
			...
			case AVM2Code.OPT_S8:
				...
				break;
			case AVM2Code.OPT_CASE_OFFSETS:
				cnt += ABCOutputStream.getU30ByteLength(operands[i]); // case
																		// count
				for (int j = i + 1; j < operands.length; j++) {
					cnt += 3;
				}
	...
	public String getParams(AVM2ConstantPool constants,
		...
		for (int i = 0; i < definition.operands.length; i++) {
			...
			case AVM2Code.DAT_NAMESPACE_INDEX:
				if (operands[i] == 0) {
					...
					s.append(" ");
					s.append(Multiname
							.namespaceToString(constants, operands[i]));
				}
				break;
			case AVM2Code.DAT_MULTINAME_INDEX:
				if (operands[i] == 0) {
					...
					if (multiname != null) {
						s.append(multiname.toString(constants,
								fullyQualifiedNames));
					} else {
				...
				/*
				 * s.append(" m["); s.append(operands[i]); s.append("]\""); if
				 * (constants.constant_multiname[operands[i]] == null) {
				 * s.append(""); } else {
				 * s.append(Helper.escapeString(constants.
				 * constant_multiname[operands[i]].toString(constants,
				 * fullyQualifiedNames))); } s.append("\"");
				 */
				break;
			case AVM2Code.DAT_STRING_INDEX:
				String str;
				if (operands[i] == 0
						|| (str = constants.getString(operands[i])) == null) {
					s.append(" null");
				} else {
					s.append(" \"");
	...
	public GraphTextWriter toString(GraphTextWriter writer, LocalData localData) {
		...
		nwriter.appendNoHilight(Helper.formatAddress(address) + " "
				+ String.format("%-30s", Helper.byteArrToString(getBytes()))
				+ definition.instructionName);
		nwriter.appendNoHilight(getParams(localData.constantsAvm2,
				localData.fullyQualifiedNames) + getComment());
		writer.marge(nwriter);
		return writer;
	}

	public String toStringNoAddress(AVM2ConstantPool constants,
			List<DottedChain> fullyQualifiedNames) {
	...
	public void translate(BaseLocalData localData, TranslateStack stack,
		...
		/*
		 * if (stack.size() != expectedSize) { throw new
	...
	public boolean isBranch() {
		return (definition instanceof IfTypeIns)
				|| (definition instanceof LookupSwitchIns);
	}

	@Override
	public boolean isExit() {
		return (definition instanceof ReturnValueIns)
				|| (definition instanceof ReturnVoidIns)
				|| (definition instanceof ThrowIns);
	}
	 ...
	 * @param body
	 */
	public void setOperand(int operandIndex, int newValue, AVM2Code code,
			MethodBody body) {

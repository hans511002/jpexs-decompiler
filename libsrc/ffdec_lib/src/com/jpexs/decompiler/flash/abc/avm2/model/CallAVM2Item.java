...
public class CallAVM2Item extends AVM2Item {
	...
	public CallAVM2Item(GraphSourceItem instruction,
			GraphSourceItem lineStartIns, GraphTargetItem receiver,
			GraphTargetItem function, List<GraphTargetItem> arguments) {
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		/*
		 * String recPart = ""; receiver.toString(constants, localRegNames) +
		 * writer.append("."); if (receiver instanceof NewActivationAVM2Item) {
		 * recPart = ""; } if (receiver instanceof ThisAVM2Item) { recPart = "";
		 * }
		 */
		if (function.getPrecedence() > precedence) {
			nwriter.append("(");
			function.toString(nwriter, localData);
			nwriter.append(")");
		} else {
			function.toString(nwriter, localData);
		}

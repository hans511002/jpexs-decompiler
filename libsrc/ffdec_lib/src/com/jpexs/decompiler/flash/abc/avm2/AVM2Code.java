...
public class AVM2Code implements Cloneable {
	...
	private static final Logger logger = Logger.getLogger(AVM2Code.class
			.getName());
	...
	private static Map<Integer, String> operandDataTypeIdentifiers = ReflectionTools
			.getConstNamesMap(AVM2Code.class, Integer.class, "^DAT_(.*)$");
	...
	public static String operandTypeToString(int ot, boolean withTypeSize) {
		...
		if (ot == OPT_CASE_OFFSETS) {
			return "number" + (withTypeSize ? "(U30)" : "") + ", offset"
					+ (withTypeSize ? "(S24)" : "") + ", offset"
					+ (withTypeSize ? "(S24)" : "") + ", ...";
		}
	...
	/* 0x22 */new PushFloatIns(), // major 47+
			...
			/* 0x33 */new PushDecimalIns(), // pushdecimal(minor 17), lix8
											// (internal-only) according to
											// Tamarin
			/* 0x34 */new PushDNanIns(), // pushdnan according to Flex SDK,
											// lix16 (internal-only) according
											// to Tamarin
			/* 0x35 */new Li8Ins(),
			/* 0x36 */new Li16Ins(),
			/* 0x37 */new Li32Ins(),
			...
			/* 0x5B */new DelDescendantsIns(), // deldescendants according to
												// Flex,
												// findpropglobalstrict(internal-only)
												// according to Tamarin
	...
	static {
		...
		for (int i = 0; i < allInstructionSet.length; i++) {
			if (allInstructionSet[i] != null) {
				...
				} else if (instructionSet[opCode]
						.hasFlag(AVM2InstructionFlag.NO_FLASH_PLAYER)
						&& !allInstructionSet[i]
								.hasFlag(AVM2InstructionFlag.NO_FLASH_PLAYER)) {
					instructionSet[opCode] = allInstructionSet[i];
				} // Prefer without decimal:
				else if (instructionSet[opCode]
						.hasFlag(AVM2InstructionFlag.ES4_NUMERICS_MINOR)
						&& !allInstructionSet[i]
								.hasFlag(AVM2InstructionFlag.ES4_NUMERICS_MINOR)) {
					instructionSet[opCode] = allInstructionSet[i];
				} // Prefer without float:
				else if (instructionSet[opCode]
						.hasFlag(AVM2InstructionFlag.FLOAT_MAJOR)
						&& !allInstructionSet[i]
								.hasFlag(AVM2InstructionFlag.FLOAT_MAJOR)) {
					instructionSet[opCode] = allInstructionSet[i];
				}
	...
	public Object execute(HashMap<Integer, Object> arguments,
			AVM2ConstantPool constants) throws AVM2ExecutionException {
		return execute(arguments, constants, null);
	}

	public Object execute(HashMap<Integer, Object> arguments,
			AVM2ConstantPool constants, AVM2RuntimeInfo runtimeInfo)
			throws AVM2ExecutionException {
		int pos = 0;
		LocalDataArea lda = new LocalDataArea();
		lda.methodName = "methodName"; // todo: needed for VerifyError exception
										// message
		...
		while (pos < code.size()) {
			...
			if (lda.jump != null) {
				try {
					...
					throw new AVM2VerifyErrorException(
							AVM2VerifyErrorException.BRANCH_TARGET_INVALID_INSTRUCTION,
							lda.isDebug());
				}
				lda.jump = null;
			} else {
	...
	private boolean calculateDebugFileLine(String debugFile, int debugLine,
			int pos, ABC abc, Set<Integer> seen) {
		while (pos < code.size()) {
			AVM2Instruction ins = code.get(pos);
			...
			if (ins.definition instanceof NewFunctionIns) {
				// Only analyze NewFunction objects that are not immediately
				// discarded by Pop.
				// This avoids bogus functions used in obfuscation or special
				// compilers that can lead to infinite recursion.
				if ((pos + 1 < code.size())
						&& !(code.get(pos + 1).definition instanceof PopIns)) {
					MethodBody innerBody = abc.findBody(ins.operands[0]);
					if (innerBody != null) { // Ignore functions without body
						innerBody.getCode().calculateDebugFileLine(debugFile,
								debugLine, 0, abc, new HashSet<>());
					}
				}
			}
			...
			if (ins.definition instanceof JumpIns) {
				...
				try {
					...
					calculateDebugFileLine(debugFile, debugLine, newpos, abc,
							seen);
				} catch (ConvertException ex) {
					return false;
				}
			...
			if (ins.definition instanceof LookupSwitchIns) {
				for (int i = 0; i < ins.operands.length; i++) {
					...
					try {
						...
						if (!calculateDebugFileLine(debugFile, debugLine,
								newpos, abc, seen)) {
							return false;
						}
					} catch (ConvertException ex) {
	...
	public void removeWrongIndices(AVM2ConstantPool constants) {
		for (AVM2Instruction ins : code) {
			for (int i = 0; i < ins.definition.operands.length; i++) {
				if (ins.definition.operands[i] == DAT_MULTINAME_INDEX
						&& ins.operands[i] >= constants.getMultinameCount()) {
					ins.operands[i] = 0;
				}
				if (ins.definition.operands[i] == DAT_DOUBLE_INDEX
						&& ins.operands[i] >= constants.getDoubleCount()) {
					ins.operands[i] = 0;
				}
				if (ins.definition.operands[i] == DAT_INT_INDEX
						&& ins.operands[i] >= constants.getIntCount()) {
					ins.operands[i] = 0;
				}
				if (ins.definition.operands[i] == DAT_UINT_INDEX
						&& ins.operands[i] >= constants.getUIntCount()) {
					ins.operands[i] = 0;
				}
				if (ins.definition.operands[i] == DAT_STRING_INDEX
						&& ins.operands[i] >= constants.getStringCount()) {
					ins.operands[i] = 0;
				}
			}
		}
	...
	public AVM2Code(ABCInputStream ais, MethodBody body) throws IOException {
		...
		// Do not add new jumps when processing these addresses (unreachable
		// code,etc.)
		List<Long> unAdresses = new ArrayList<>();
		// Handle lookupswitches at the end - they can be invalid. Handle other
		// instruction first so we can decide lookupswitch to be invalid based
		// on other instructions inside it
		// Flashplayer does not check casecount in lookupswitch instruction so
		// the instruction can "be" long and over other instructions
		List<Long> switchAddresses = new ArrayList<>();
		int availableBytes = ais.available();
		for (int i = 0; i < availableBytes; i++) {
			codeMap.put((long) i, new AVM2Instruction(i, AVM2Instructions.Nop,
					null));
		}
		...
		loopaddr: while (!addresses.isEmpty() || !switchAddresses.isEmpty()
			...
			try {
				...
				while (ais.available() > 0) {
					...
					if (codeMap.containsKey(startOffset)
							&& !(codeMap.get(startOffset).definition instanceof NopIns)) {
						continue loopaddr;
					}

					DumpInfo di = ais
							.newDumpLevel("instruction", "instruction");
					...
					try {
						...
						if (instr != null) {
							...
							if (instructionCode == AVM2Instructions.LookupSwitch) { // switch
								int firstOperand = ais
										.readS24("default_offset");
								int case_count = ais.readU30("case_count");
								long afterCasePos = ais.getPosition() + 3
										* (case_count + 1);

								boolean invalidSwitch = false;
								// If there are already some instructions in the
								// lookupswitch bytes, the lookupswitch is
								// invalid (obfuscation)
								for (long a = startOffset; a < afterCasePos; a++) {
									if (codeMap.containsKey(a)
											&& (!(codeMap.get(a).definition instanceof NopIns))) {
										invalidSwitch = true;
										break;
									}
								}

								long totalBytes = ais.getPosition()
										+ ais.available();

								// If the lookupswitch case_count are larger
								// than available bytes, the lookupswitch is
								// invalid (obfuscation)
								...
								if (invalidSwitch) {
									...
									for (int c = 0; c < case_count + 1; c++) {
										actualOperands[2 + c] = ais
												.readS24("actualOperand");
									}
								}
								...
								for (int op = 0; op < instr.operands.length; op++) {
									...
									case OPT_U30:
										actualOperands[op] = ais
												.readU30("operand");
										break;
									case OPT_S16:
										actualOperands[op] = (short) ais
												.readU30("operand");
										break;
									case OPT_U8:
										actualOperands[op] = ais
							...
							AVM2Instruction ai = new AVM2Instruction(
									startOffset, instr, actualOperands);
							long endOffset = ais.getPosition();

							boolean hasRoom = true;
							for (long p = startOffset; p < endOffset; p++) {
								if (codeMap.containsKey(p)
										&& !(codeMap.get(p).definition instanceof NopIns)) {
									hasRoom = false;
								}
							}

							// There is no room for this instruction (it is
							// invalid?)
							if (!hasRoom) {
								continue loopaddr;
							}
							...
							if (instr.isExitInstruction()) { // do not process
																// jumps if
																// there is
																// return/throw
																// instruction
							...
							break; // Unknown instructions are ignored (Some of
									// the obfuscators add unknown instructions)
							// throw new
							// UnknownInstructionCode(instructionCode);
						}
					} finally {
						if (instr == null) {
							ais.endDumpLevel();
	...
	public void setInstructionOperand(int ip, int operandIndex, int value,
			MethodBody body) {
	...
	public GraphTextWriter toASMSource(AVM2ConstantPool constants,
			MethodInfo info, MethodBody body, ScriptExportMode exportMode,
			GraphTextWriter writer) {
		return toASMSource(constants, info, body, new ArrayList<>(),
				exportMode, writer);
	}
	...
	public GraphTextWriter toASMSource(AVM2ConstantPool constants,
		...
		if (info != null) {
			...
			nwriter.appendNoHilight("name ");
			nwriter.hilightSpecial(info.name_index == 0 ? "null" : "\""
					+ Helper.escapeActionScriptString(info.getName(constants))
					+ "\"", HighlightSpecialType.METHOD_NAME);
			nwriter.newLine();
			if (info.flagExplicit()) {
				nwriter.appendNoHilight("flag ");
				nwriter.hilightSpecial("EXPLICIT",
						HighlightSpecialType.FLAG_EXPLICIT);
				nwriter.newLine();
			}
			if (info.flagHas_optional()) {
				nwriter.appendNoHilight("flag ");
				nwriter.hilightSpecial("HAS_OPTIONAL",
						HighlightSpecialType.FLAG_HAS_OPTIONAL);
				nwriter.newLine();
			}
			if (info.flagHas_paramnames()) {
				nwriter.appendNoHilight("flag ");
				nwriter.hilightSpecial("HAS_PARAM_NAMES",
						HighlightSpecialType.FLAG_HAS_PARAM_NAMES);
				nwriter.newLine();
			}
			if (info.flagIgnore_rest()) {
				nwriter.appendNoHilight("flag ");
				nwriter.hilightSpecial("EXPLICIT",
						HighlightSpecialType.FLAG_IGNORE_REST);
				nwriter.newLine();
			}
			if (info.flagNeed_activation()) {
				nwriter.appendNoHilight("flag ");
				nwriter.hilightSpecial("NEED_ACTIVATION",
						HighlightSpecialType.FLAG_NEED_ACTIVATION);
				nwriter.newLine();
			}
			if (info.flagNeed_arguments()) {
				nwriter.appendNoHilight("flag ");
				nwriter.hilightSpecial("NEED_ARGUMENTS",
						HighlightSpecialType.FLAG_NEED_ARGUMENTS);
				nwriter.newLine();
			}
			if (info.flagNeed_rest()) {
				nwriter.appendNoHilight("flag ");
				nwriter.hilightSpecial("NEED_REST",
						HighlightSpecialType.FLAG_NEED_REST);
				nwriter.newLine();
			}
			if (info.flagSetsdxns()) {
				nwriter.appendNoHilight("flag ");
				nwriter.hilightSpecial("SET_DXNS",
						HighlightSpecialType.FLAG_SET_DXNS);
				nwriter.newLine();
			}
			for (int p = 0; p < info.param_types.length; p++) {
				nwriter.appendNoHilight("param ");
				nwriter.hilightSpecial(
						constants.multinameToString(info.param_types[p]),
						HighlightSpecialType.PARAM, p);
				nwriter.newLine();
			}
			if (info.flagHas_paramnames()) {
				for (int n : info.paramNames) {
					nwriter.appendNoHilight("paramname ");
					if (n == 0) {
						nwriter.appendNoHilight("null");
					} else {
						nwriter.appendNoHilight("\"");
						nwriter.appendNoHilight(constants.getString(n));
						nwriter.appendNoHilight("\"");
					}
					nwriter.newLine();
				}
			...
			if (info.flagHas_optional()) {
				for (int i = 0; i < info.optional.length; i++) {
					...
					nwriter.appendNoHilight("optional ");
					nwriter.hilightSpecial(vk.toString(constants),
							HighlightSpecialType.OPTIONAL, i);
					nwriter.newLine();
				}
			}
			nwriter.appendNoHilight("returns ");
			nwriter.hilightSpecial(constants.multinameToString(info.ret_type),
					HighlightSpecialType.RETURNS);
			nwriter.newLine();
		}
		nwriter.newLine();
		...
		if (body != null) {
			nwriter.appendNoHilight("body").newLine();

			nwriter.appendNoHilight("maxstack ");
			nwriter.appendNoHilight(body.max_stack);
			nwriter.newLine();

			nwriter.appendNoHilight("localcount ");
			nwriter.appendNoHilight(body.max_regs);
			nwriter.newLine();

			nwriter.appendNoHilight("initscopedepth ");
			nwriter.appendNoHilight(body.init_scope_depth);
			nwriter.newLine();

			nwriter.appendNoHilight("maxscopedepth ");
			nwriter.appendNoHilight(body.max_scope_depth);
			nwriter.newLine();

			for (int e = 0; e < body.exceptions.length; e++) {
				ABCException exception = body.exceptions[e];
				nwriter.appendNoHilight("try");

				nwriter.appendNoHilight(" from ");
				nwriter.appendNoHilight("ofs");
				nwriter.appendNoHilight(Helper.formatAddress(exception.start));

				nwriter.appendNoHilight(" to ");
				nwriter.appendNoHilight("ofs");
				nwriter.appendNoHilight(Helper.formatAddress(exception.end));

				nwriter.appendNoHilight(" target ");
				nwriter.appendNoHilight("ofs");
				nwriter.appendNoHilight(Helper.formatAddress(exception.target));

				nwriter.appendNoHilight(" type ");
				nwriter.hilightSpecial(exception.type_index == 0 ? "null"
						: constants.getMultiname(exception.type_index)
								.toString(constants, new ArrayList<>()),
						HighlightSpecialType.TRY_TYPE, e);

				nwriter.appendNoHilight(" name ");
				nwriter.hilightSpecial(exception.name_index == 0 ? "null"
						: constants.getMultiname(exception.name_index)
								.toString(constants, new ArrayList<>()),
						HighlightSpecialType.TRY_NAME, e);
				nwriter.newLine();
			}
		}

		nwriter.newLine();
		nwriter.appendNoHilight("code").newLine();
		int ip = 0;
		int largeLimit = 20000;
		boolean markOffsets = code.size() <= largeLimit;

		if (exportMode == ScriptExportMode.HEX) {
			Helper.byteArrayToHexWithHeader(nwriter, getBytes());
		} else if (exportMode == ScriptExportMode.PCODE
				|| exportMode == ScriptExportMode.PCODE_HEX) {
			for (AVM2Instruction ins : code) {
				long addr = ins.getAddress();
				if (exportMode == ScriptExportMode.PCODE_HEX) {
					nwriter.appendNoHilight("; ");
					nwriter.appendNoHilight(Helper.bytesToHexString(ins
							.getBytes()));
					nwriter.newLine();
				}
				if (Configuration.showAllAddresses.get()
						|| importantOffsets.contains(addr)) {
					nwriter.appendNoHilight("ofs" + Helper.formatAddress(addr)
							+ ":");
				}
				/*
				 * for (int e = 0; e < body.exceptions.length; e++) { if
				 * (body.exceptions[e].start == ofs) {
				 * ret.append("exceptionstart " + e + ":"); } if
				 * (body.exceptions[e].end == ofs) { ret.append("exceptionend "
				 * + e + ":"); } if (body.exceptions[e].target == ofs) {
				 * ret.append("exceptiontarget " + e + ":"); } }
				 */

				if (!ins.isIgnored()) {
					if (markOffsets) {
						nwriter.append("", addr, ins.getFileOffset());
					}

					nwriter.appendNoHilight(ins.toStringNoAddress(constants,
							new ArrayList<>()));
					nwriter.newLine();
					outputMap.add(ip);
				}
			...
			nwriter.appendNoHilight("Constant export mode is not supported.")
					.newLine();
		}
		nwriter.appendNoHilight("end ; code").newLine();
		if (body != null) {
			nwriter.appendNoHilight("end ; body").newLine();
		}
		if (info != null) {
	...
	public int adr2pos(long address, boolean nearest) throws ConvertException {
		...
		if (ret < 0) {
			...
			throw new ConvertException("Invalid jump to ofs"
					+ Helper.formatAddress(address), -1);
		}
		return ret;
	}
		...
		for (AVM2Instruction ins : code) {
			if (ins.definition instanceof DebugIns) {
				if (ins.operands[0] == 1) {
					...
					// Same name already exists, it may be wrong names inserted
					// by obfuscator
	...
	public List<GraphTargetItem> clearTemporaryRegisters(
			List<GraphTargetItem> input) {
		List<GraphTargetItem> output = new ArrayList<>(input);
		for (int i = 0; i < output.size(); i++) {
			if (output.get(i) instanceof SetLocalAVM2Item) {
				if (isKilled(((SetLocalAVM2Item) output.get(i)).regIndex, 0,
						code.size() - 1)) {
	...
	public ConvertOutput toSourceOutput(boolean thisHasDefaultToPrimitive,
			Reference<GraphSourceItem> lineStartItem, String path,
			GraphPart part, boolean processJumps, boolean isStatic,
			int scriptIndex, int classIndex,
			HashMap<Integer, GraphTargetItem> localRegs, TranslateStack stack,
			ScopeStack scopeStack, ABC abc, MethodBody body, int start,
			int end, HashMap<Integer, String> localRegNames,
			List<DottedChain> fullyQualifiedNames, boolean[] visited,
		...
		if (debugMode) {
			System.err.println("OPEN SubSource:" + start + "-" + end + " "
					+ code.get(start).toString() + " to "
					+ code.get(end).toString());
		}
		if (visited == null) {
		...
		if (toSourceLimit > 0 && toSourceCount >= toSourceLimit) {
			throw new ConvertException("Limit of subs(" + toSourceLimit
					+ ") was reached", start);
		}
		...
		iploop: while (ip <= end) {
			...
			if (visited[ip]) {
				// logger.warning(path + ": Code already visited, ofs:" +
				// Helper.formatAddress(pos2adr(ip)) + ", ip:" + ip);
				break;
			}
			...
			if (debugMode) {
				System.err.println("translating ip " + ip + " ins "
						+ ins.toString() + " stack:" + stack.toString()
						+ " scopeStack:" + scopeStack.toString());
			}
			if (ins.definition instanceof NewFunctionIns) {
				if (ip + 1 <= end) {
			...
			/*
			 * if ((ip + 8 < code.size())) { //return in finally clause if
			 * (ins.definition instanceof SetLocalTypeIns) { if (code.get(ip +
			 * 1).definition instanceof PushByteIns) { AVM2Instruction jmp =
			 * code.get(ip + 2); if (jmp.definition instanceof JumpIns) { if
			 * (jmp.operands[0] == 0) { if (code.get(ip + 3).definition
			 * instanceof LabelIns) { if (code.get(ip + 4).definition instanceof
			 * PopIns) { if (code.get(ip + 5).definition instanceof LabelIns) {
			 * AVM2Instruction gl = code.get(ip + 6); if (gl.definition
			 * instanceof GetLocalTypeIns) { if (((GetLocalTypeIns)
			 * gl.definition).getRegisterId(gl) == ((SetLocalTypeIns)
			 * ins.definition).getRegisterId(ins)) { AVM2Instruction ki =
			 * code.get(ip + 7); if (ki.definition instanceof KillIns) { if
			 * (ki.operands[0] == ((SetLocalTypeIns)
			 * ins.definition).getRegisterId(ins)) { if (code.get(ip +
			 * 8).definition instanceof ReturnValueIns) { ip = ip + 8; continue;
			 * } } } } } } } } } } } } }//
			 */

			/*
			 * if ((ip + 2 < code.size()) && (ins.definition instanceof
			 * NewCatchIns)) { // Filling local register in catch clause if
			 * (code.get(ip + 1).definition instanceof DupIns) { if (code.get(ip
			 * + 2).definition instanceof SetLocalTypeIns) {
			 * ins.definition.translate(isStatic, classIndex, localRegs, stack,
			 * scopeStack, constants, ins, method_info, output, body, abc,
			 * localRegNames, fullyQualifiedNames); ip += 3; continue; } } }
			 */
			if ((ins.definition instanceof GetLocalTypeIns)
					&& (!output.isEmpty())
					&& (output.get(output.size() - 1) instanceof SetLocalAVM2Item)
					&& (((SetLocalAVM2Item) output.get(output.size() - 1)).regIndex == ((GetLocalTypeIns) ins.definition)
							.getRegisterId(ins))
					&& isKilled(
							((SetLocalAVM2Item) output.get(output.size() - 1)).regIndex,
							start, end)) {
				SetLocalAVM2Item slt = (SetLocalAVM2Item) output.remove(output
						.size() - 1);
				...
				ip++;
					...
					&& (ip + 1 <= end)
					&& (isKilled(
							((SetLocalTypeIns) ins.definition)
									.getRegisterId(ins),
							ip, end))) { // set_local_x,get_local_x..kill x
				AVM2Instruction insAfter = code.get(ip + 1);
				if ((insAfter.definition instanceof GetLocalTypeIns)
						&& (((GetLocalTypeIns) insAfter.definition)
								.getRegisterId(insAfter) == ((SetLocalTypeIns) ins.definition)
								.getRegisterId(ins))) {
					GraphTargetItem before = stack.peek();
					ins.definition.translate(lineStartItem, isStatic,
							scriptIndex, classIndex, localRegs, stack,
							scopeStack, ins, output, body, abc, localRegNames,
							fullyQualifiedNames, path, localRegAssigmentIps,
							ip, refs, this, thisHasDefaultToPrimitive);
					stack.push(before);
					ip += 2;
					continue iploop;
				} else {
				...
				int nextPos;
				do {
					AVM2Instruction insAfter = ip + 1 < code.size() ? code
							.get(ip + 1) : null;
					if (insAfter == null) {
						ins.definition.translate(lineStartItem, isStatic,
								scriptIndex, classIndex, localRegs, stack,
								scopeStack, ins, output, body, abc,
								localRegNames, fullyQualifiedNames, path,
								localRegAssigmentIps, ip, refs, this,
								thisHasDefaultToPrimitive);
						ip++;
						break;
					}
					...
					if (insAfter.definition instanceof ConvertBIns) { // SWF
																		// compiled
																		// with
																		// debug
																		// contain
					...
					boolean isAnd;
					if (processJumps
							&& (insAfter.definition instanceof IfFalseIns)) {
						// stack.add("(" + stack.pop() + ")&&");
						isAnd = true;
					} else if (processJumps
							&& (insAfter.definition instanceof IfTrueIns)) {
						// stack.add("(" + stack.pop() + ")||");
						isAnd = false;
					} else if (insAfter.definition instanceof SetLocalTypeIns) {
						// chained assignments
						int reg = (((SetLocalTypeIns) insAfter.definition)
								.getRegisterId(insAfter));
						for (int t = ip + 1; t <= end - 1; t++) {
							...
							if (code.get(t).definition instanceof GetLocalTypeIns) {
								if (((GetLocalTypeIns) code.get(t).definition)
										.getRegisterId(code.get(t)) == reg) {
									if (code.get(t + 1).definition instanceof KillIns) {
										if (code.get(t + 1).operands[0] == reg) {
											ConvertOutput assignment = toSourceOutput(
													thisHasDefaultToPrimitive,
													lineStartItem, path, part,
													processJumps, isStatic,
													scriptIndex, classIndex,
													localRegs, stack,
													scopeStack, abc, body,
													ip + 2, t - 1,
													localRegNames,
													fullyQualifiedNames,
													visited,
													localRegAssigmentIps, refs);
											if (!assignment.output.isEmpty()) {
												GraphTargetItem tar = assignment.output
						...
						if (!isKilled(reg, 0, end)) {
							...
							for (int i = 0; i < dupCnt; i++) {
								stack.push(new LocalRegAVM2Item(ins,
										(AVM2Instruction) lineStartItem
												.getVal(), reg, vx));
							}
							stack.push(vx);
						} else {
						...
						break;
						// throw new
						// ConvertException("Unknown pattern after DUP:" +
						// insComparsion.toString());
					}
				} while (ins.definition instanceof DupIns);
			} else if ((ins.definition instanceof ReturnValueIns)
					|| (ins.definition instanceof ReturnVoidIns)
					|| (ins.definition instanceof ThrowIns)) {
				ins.definition.translate(lineStartItem, isStatic, scriptIndex,
						classIndex, localRegs, stack, scopeStack, ins, output,
						body, abc, localRegNames, fullyQualifiedNames, path,
						localRegAssigmentIps, ip, refs, this,
						thisHasDefaultToPrimitive);
				// ip = end + 1;
				break;
				...
				try {
					...
					/*
					 * String last=""; int len=5; for(int
					 * i=(ip-len<0?0:ip-len);i<ip+len && ip<code.size();i++){
					 * if(i==ip){ last+=">"; }
					 * last+=""+i+": "+code.get(i).toString()+"\r\n"; }
					 * Logger.getLogger
					 * (AVM2Code.class.getName()).log(Level.SEVERE,
					 * "ins list:\r\n{0}", last);
					 */
					throw re;
				}
		...
		if (debugMode) {
			System.err.println("CLOSE SubSource:" + start + "-" + end + " "
					+ code.get(start).toString() + " to "
					+ code.get(end).toString());
		}
		/*
		 * if (hideTemporaryRegisters) { clearTemporaryRegisters(output); }
		 */
		return new ConvertOutput(stack, output);
		/*
		 * } catch (ConvertException cex) { throw cex; }
		 */
	}
	...
	public HashMap<Integer, GraphTargetItem> getLocalRegTypes(
			...
			if (ins.definition instanceof SetLocalTypeIns) {
				if (prev != null) {
					if (prev.definition instanceof CoerceOrConvertTypeIns) {
						ret.put(((SetLocalTypeIns) ins.definition)
								.getRegisterId(ins),
								((CoerceOrConvertTypeIns) prev.definition)
										.getTargetType(constants, prev));
					}
				}
			}
			prev = ins;
	...
	private class Slot {
		...
		public boolean equals(Object obj) {
			if (obj instanceof Slot) {
				Slot slot = (Slot) obj;
				return (slot.scope.getThroughRegister() == scope
						.getThroughRegister()) && (slot.multiname == multiname);
			}
			return false;
		}
		...
		public int hashCode() {
			...
			hash = 59
					* hash
					+ (scope != null ? Objects.hashCode(scope
							.getThroughRegister()) : 0);
	...
	private GraphTargetItem handleDeclareReg(int minreg,
			GraphTargetItem assignment,
			DeclarationAVM2Item[] declaredRegisters, List<Slot> declaredSlots,
			int reg) {

		// do not add declarations for reserved local registers like function
		// arguments
		if (reg < minreg) {
			return assignment;
		}
		...
		if (vtype.equals(TypeItem.UNBOUNDED)
				&& (assignment.value instanceof CoerceAVM2Item)) {
			vtype = ((CoerceAVM2Item) assignment.value).typeObj;
		}
		if (vtype.equals(TypeItem.UNBOUNDED)
				&& (assignment.value instanceof SimpleValue)
				&& ((SimpleValue) assignment.value).isSimpleValue()) {
		...
		if (declaredRegisters[reg] == null) {
			...
			if (assignment instanceof SetTypeAVM2Item) {
				((SetTypeAVM2Item) assignment)
						.setDeclaration(declaredRegisters[reg]);
			}
		...
		} else if (!declaredRegisters[reg].type.equals(vtype)) { // already
																	// declared
																	// with
																	// different
																	// type
	...
	private GraphTargetItem injectDeclarations(int minreg, GraphTargetItem ti,
			DeclarationAVM2Item[] declaredRegisters, List<Slot> declaredSlots,
			List<DeclarationAVM2Item> declaredSlotsDec, ABC abc, MethodBody body) {
		if (ti.value != null) {
			ti.value = injectDeclarations(minreg, ti.value, declaredRegisters,
					declaredSlots, declaredSlotsDec, abc, body);
		}
		// TODO: walk whole tree... some walker?
		if (ti instanceof IfItem) {
			((IfItem) ti).expression = injectDeclarations(minreg,
					((IfItem) ti).expression, declaredRegisters, declaredSlots,
					declaredSlotsDec, abc, body);
		}
		if (ti instanceof BinaryOpItem) {
			((BinaryOpItem) ti).leftSide = injectDeclarations(minreg,
					((BinaryOpItem) ti).leftSide, declaredRegisters,
					declaredSlots, declaredSlotsDec, abc, body);
			((BinaryOpItem) ti).rightSide = injectDeclarations(minreg,
					((BinaryOpItem) ti).rightSide, declaredRegisters,
					declaredSlots, declaredSlotsDec, abc, body);
		}
		if (ti instanceof ForEachInAVM2Item) {
			...
			if (fei.expression.object instanceof LocalRegAVM2Item) {
				...
				if (declaredRegisters[reg] == null) {
					fei.expression.object = handleDeclareReg(minreg,
							fei.expression.object, declaredRegisters,
							declaredSlots, reg);
				}
			}
		}
		if (ti instanceof ForInAVM2Item) {
			...
			if (fi.expression.object instanceof LocalRegAVM2Item) {
				...
				fi.expression.object = handleDeclareReg(minreg,
						fi.expression.object, declaredRegisters, declaredSlots,
						reg);
		...
		if (ti instanceof Block) {
			...
			for (List<GraphTargetItem> s : bl.getSubs()) {
				injectDeclarations(minreg, s, declaredRegisters, declaredSlots,
						declaredSlotsDec, abc, body);
			}
		}
		if (ti instanceof SetLocalAVM2Item) {
			int reg = ((SetLocalAVM2Item) ti).regIndex;
			ti = handleDeclareReg(minreg, ti, declaredRegisters, declaredSlots,
					reg);
			return ti;
		}
				...
				for (int t = 0; t < body.traits.traits.size(); t++) {
					if (body.traits.traits.get(t).getName(abc) == sl.multiname) {
						if (body.traits.traits.get(t) instanceof TraitSlotConst) {
							type = PropertyAVM2Item
									.multinameToType(
											((TraitSlotConst) body.traits.traits
													.get(t)).type_index,
											abc.constants);
						}
					}
	...
	private void injectDeclarations(int minreg, List<GraphTargetItem> list,
			DeclarationAVM2Item[] declaredRegisters, List<Slot> declaredSlots,
			List<DeclarationAVM2Item> declaredSlotsDec, ABC abc, MethodBody body) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			GraphTargetItem ti2 = injectDeclarations(minreg, ti,
					declaredRegisters, declaredSlots, declaredSlotsDec, abc,
					body);
			if (ti != ti2) {
				list.set(i, ti2);
			}
		...
		/*
		 * //undeclare registers at the end of the block? for(int
		 * reg:nowdeclaredRegs){ declaredRegisters[reg] = false; }
		 * 
		 * for(Slot s:nowdeclaredSlots){ declaredSlots.remove(s); }
		 */
	}

	public List<GraphTargetItem> toGraphTargetItems(
			boolean thisHasDefaultToPrimitive, ConvertData convertData,
			String path, int methodIndex, boolean isStatic, int scriptIndex,
			int classIndex, ABC abc, MethodBody body,
			HashMap<Integer, String> localRegNames, ScopeStack scopeStack,
			int initializerType, List<DottedChain> fullyQualifiedNames,
			List<Traits> initTraits, int staticOperation,
			HashMap<Integer, Integer> localRegAssigmentIps,
		...
		// try {
		list = AVM2Graph.translateViaGraph(path, this, abc, body, isStatic,
				scriptIndex, classIndex, localRegs, scopeStack, localRegNames,
				fullyQualifiedNames, staticOperation, localRegAssigmentIps,
				refs, thisHasDefaultToPrimitive);

		if (initTraits != null) {
			loopi: for (int i = 0; i < list.size(); i++) {
				GraphTargetItem ti = list.get(i);
				if ((ti instanceof InitPropertyAVM2Item)
						|| (ti instanceof SetPropertyAVM2Item)) {
					...
					for (Traits ts : initTraits) {
						for (Trait t : ts.traits) {
							Multiname tm = abc.constants
									.getMultiname(t.name_index);
							if (tm != null && tm.equals(m)) {
								if ((t instanceof TraitSlotConst)) {
									if (((TraitSlotConst) t).isConst()
											|| initializerType == GraphTextWriter.TRAIT_CLASS_INITIALIZER
											|| initializerType == GraphTextWriter.TRAIT_SCRIPT_INITIALIZER) {
										TraitSlotConst tsc = (TraitSlotConst) t;
										if (value != null
												&& !convertData.assignedValues
														.containsKey(tsc)) {

											if (value instanceof NewFunctionAVM2Item) {
												NewFunctionAVM2Item f = (NewFunctionAVM2Item) value;
												f.functionName = tsc.getName(
														abc).getName(
														abc.constants,
														fullyQualifiedNames,
														true, true);
											}
											AssignedValue av = new AssignedValue(
													value, initializerType,
													methodIndex);
											convertData.assignedValues.put(tsc,
													av);
											list.remove(i);
											i--;
											continue;
		...
		if (initializerType == GraphTextWriter.TRAIT_CLASS_INITIALIZER
				|| initializerType == GraphTextWriter.TRAIT_SCRIPT_INITIALIZER) {
		...
		for (int i = 0; i < param_types.length; i++) {
			...
			if (param_types[i] == 0) {
				...
				type = new TypeItem(abc.constants.getMultiname(param_types[i])
						.getNameWithNamespace(abc.constants, true));
			}
			if (d.length > r) {
				d[r] = new DeclarationAVM2Item(new SetLocalAVM2Item(null, null,
						r, new NullAVM2Item(null, null)), type);
			}
			r++;
		}
		if (abc.method_info.get(body.method_info).flagNeed_arguments()) {
			if (d.length > r) {
				d[r] = new DeclarationAVM2Item(new SetLocalAVM2Item(null, null,
						r, new NullAVM2Item(null, null)), TypeItem.ARRAY /* ? */);
			}
			r++;
		}
		...
		// int minreg =
		// abc.method_info.get(body.method_info).getMaxReservedReg() + 1;
		injectDeclarations(1, list, d, new ArrayList<>(), new ArrayList<>(),
				abc, body);
		...
		if ((list.size() > lastPos)
				&& (list.get(lastPos) instanceof ScriptEndItem)) {
			lastPos--;
		}
		if (lastPos < 0) {
			lastPos = 0;
		...
		if ((list.size() > lastPos)
				&& (list.get(lastPos) instanceof ReturnVoidAVM2Item)) {
			list.remove(lastPos);
		}
	...
	public void updateInstructionByteCountByAddr(long instructionAddress,
			int byteDelta, MethodBody body) {
		if (byteDelta != 0) {
			updateOffsets(new OffsetUpdater() {
				...
				@Override
				public int updateOperandOffset(long insAddr,
						long targetAddress, int offset) {
					if (targetAddress > instructionAddress
							&& insAddr <= instructionAddress) {
	...
	public void updateInstructionByteCount(int pos, int byteDelta,
			MethodBody body) {
		AVM2Instruction instruction = code.get(pos);
		updateInstructionByteCountByAddr(instruction.getAddress(), byteDelta,
				body);
	}
	...
	public void updateOffsets(OffsetUpdater updater, MethodBody body) {
		for (int i = 0; i < code.size(); i++) {
			...
			if (ins.definition instanceof LookupSwitchIns) {
				...
				ins.operands[0] = updater.updateOperandOffset(ins.getAddress(),
						target, ins.operands[0]);
				for (int k = 2; k < ins.operands.length; k++) {
					target = ins.getAddress() + ins.operands[k];
					ins.operands[k] = updater.updateOperandOffset(
							ins.getAddress(), target, ins.operands[k]);
				}
			} else /*
					 * for (int j = 0; j < ins.definition.operands.length; j++)
					 * { if (ins.definition.operands[j] == AVM2Code.DAT_OFFSET)
					 * { long target = ins.offset + ins.getBytes().length +
					 * ins.operands[j]; ins.operands[j] =
					 * updater.updateOperandOffset(target, ins.operands[j]); } }
					 */// Faster, but not so universal
			if (ins.definition instanceof IfTypeIns) {
				long target = ins.getTargetAddress();
				try {
					ins.operands[0] = updater.updateOperandOffset(
							ins.getAddress(), target, ins.operands[0]);
				} catch (ConvertException cex) {
					throw new ConvertException("Invalid offset (" + ins + ")",
							i);
				}
			}
			ins.setAddress(updater.updateInstructionOffset(ins.getAddress()));
			// Note: changing operands here does not change instruction byte
			// length as offsets are always S24 (not variable length)
		}
		...
		if (body != null) {
			for (ABCException ex : body.exceptions) {
				...
				ex.target = updater.updateOperandOffset(-1, ex.target,
						ex.target);
			}
		}
	}

	public void fixJumps(final String path, MethodBody body)
		...
		updateOffsets(new OffsetUpdater() {
			...
			@Override
			public int updateOperandOffset(long insAddr, long targetAddress,
					int offset) {
				if (targetAddress > endOffset || targetAddress < 0
						|| adr2posNoEx(targetAddress) < 0) {
		...
		if (someIgnored) {
			logger.log(
					Level.WARNING,
					path
							+ ": One or more invalid jump offsets found in the code. Those instructions were ignored.");
		}
		removeIgnored(body);
	}
	...
	public void removeInstruction(int pos, MethodBody body) {
		...
		updateOffsets(new OffsetUpdater() {
			...
			@Override
			public int updateOperandOffset(long jumpInsAddr,
					long jumpTargetAddr, int jumpOffset) {
				/*
				 * a:jump d: b: c:X d:
				 */
	 ...
	 * @param instruction
	 *            Instruction False means before new instruction
	 * @param body
	...
	public void replaceInstruction(int pos, AVM2Instruction instruction,
		...
		if (byteDelta != 0) {
			updateOffsets(new OffsetUpdater() {
				...
				public int updateOperandOffset(long insAddr,
						long targetAddress, int offset) {
					if (targetAddress > instruction.getAddress()
	 ...
	 * @param instruction
	 *            Instruction
	 * @param mapOffsetsAfterIns
	 *            Map all jumps to the pos after new instruction? False means
	 *            before new instruction
	 * @param body
	 *            Method body (used for try handling)
	 */
	public void insertInstruction(int pos, AVM2Instruction instruction,
			boolean mapOffsetsAfterIns, MethodBody body) {
		// checkValidOffsets(body);
		if (pos < 0) {
			pos = 0;
		}
		...
		if (code.size() == 0) {
			...
			instruction.setAddress(code.get(pos - 1).getAddress()
					+ code.get(pos - 1).getBytesLength());
		} else {
	...
	public int removeTraps(Trait trait, int methodInfo, MethodBody body,
			ABC abc, int scriptIndex, int classIndex, boolean isStatic,
			String path) throws InterruptedException {
		SWFDecompilerPlugin.fireAvm2CodeRemoveTraps(path, classIndex, isStatic,
				scriptIndex, abc, trait, methodInfo, body);
		try (Statistics s = new Statistics("AVM2DeobfuscatorGetSet")) {
			new AVM2DeobfuscatorGetSet().avm2CodeRemoveTraps(path, classIndex,
					isStatic, scriptIndex, abc, trait, methodInfo, body);
		}
		try (Statistics s = new Statistics("AVM2DeobfuscatorSimple")) {
			new AVM2DeobfuscatorSimpleOld().avm2CodeRemoveTraps(path,
					classIndex, isStatic, scriptIndex, abc, trait, methodInfo,
					body);
		}
		try (Statistics s = new Statistics("AVM2DeobfuscatorRegisters")) {
			new AVM2DeobfuscatorRegistersOld().avm2CodeRemoveTraps(path,
					classIndex, isStatic, scriptIndex, abc, trait, methodInfo,
					body);
		}
		try (Statistics s = new Statistics("AVM2DeobfuscatorJumps")) {
			new AVM2DeobfuscatorJumps().avm2CodeRemoveTraps(path, classIndex,
					isStatic, scriptIndex, abc, trait, methodInfo, body);
		}
		return 1;
	}
	...
	private boolean walkCode(CodeStats stats, int pos, int stack, int scope,
			ABC abc) {
		while (pos < code.size()) {
			AVM2Instruction ins = code.get(pos);
			...
			if (ins.definition instanceof NewFunctionIns) {
				MethodBody innerBody = abc.findBody(ins.operands[0]);
				innerBody.autoFillStats(abc, stats.initscope
						+ (stats.has_activation ? 1 : 0), false);
			}
			...
			if ((ins.definition instanceof DXNSIns)
					|| (ins.definition instanceof DXNSLateIns)) {
			...
			if (ins.definition instanceof SetLocalTypeIns) {
				handleRegister(stats,
						((SetLocalTypeIns) ins.definition).getRegisterId(ins));
			} else if (ins.definition instanceof GetLocalTypeIns) {
				handleRegister(stats,
						((GetLocalTypeIns) ins.definition).getRegisterId(ins));
			} else {
				for (int i = 0; i < ins.definition.operands.length; i++) {
	...
	public CodeStats getStats(ABC abc, MethodBody body, int initScope) {
		...
		for (int e = 0; e < body.exceptions.length; e++) {
			...
			try {
				...
				if (!walkCode(stats, adr2pos(ex.target),
						1 + (ex.isFinally() ? 1 : 0), scopePos, abc)) {
					return null;
				}
				int maxIp = 0;
				// searching for visited instruction in second run which has
				// maximum position
				for (int i = 0; i < stats.instructionStats.length; i++) {
				...
				if (code.get(maxIp).definition instanceof JumpIns) {
					nextIp = adr2pos(pos2adr(nextIp)
							+ code.get(maxIp).operands[0]);
				}
				if (nextIp < stats.instructionStats.length) {
					...
					if (prevStart == ex.start && ex.isFinally()
							&& !code.get(nextIp).isExit() && nextIpStat.seen) {
						for (int i = 0; i < stats.instructionStats.length; i++) {
							stats.instructionStats[i].seen = false;
						}
						// Rerun rest with new scopePos, stackPos
						if (!walkCode(stats, nextIp,
								origStackPos + 1/* magic! */,
								scopePos - 1 /* magic! */, abc)) {
							return null;
						}
						scopePos--;
					}
	...
	// simplified version of getStats. This method calculates only the maxlocal
	// value
	...
	private void visitCode(int ip, int lastIp,
			HashMap<Integer, List<Integer>> refs) throws InterruptedException {
	...
	public HashMap<Integer, List<Integer>> visitCode(MethodBody body)
		...
		for (ABCException e : body.exceptions) {
			pos++;
			try {
				visitCode(adr2pos(e.start, true), adr2pos(e.start, true) - 1,
						refs);
			...
			if (ins.definition instanceof JumpIns) {
				...
				try {
					...
					if (ni < code.size() && ni > -1) {
						...
						if (ins2.isExit()) {
							code.set(i, new AVM2Instruction(ofs,
									ins2.definition, ins2.operands));
							modified = true;
						}
					}
				} catch (ConvertException ex) {
	...
	private static int getMostCommonIp(AVM2GraphSource code,
	...
	public static void getReachableIps(AVM2GraphSource code, int ip,
	...
	public static boolean isDirectAncestor(int currentIp, int ancestor,
			HashMap<Integer, List<Integer>> refs) {
		return isDirectAncestor(currentIp, ancestor, refs, new ArrayList<>());
	}

	private static boolean isDirectAncestor(int currentIp, int ancestor,
			HashMap<Integer, List<Integer>> refs, List<Integer> visited) {
		if (currentIp == -1) {
			return true;
		}
		do {
			...
			if (refs.containsKey(currentIp)) {
				...
				if ((currentRefs != null) && (!currentRefs.isEmpty())) {
					for (int i = 1; i < currentRefs.size(); i++) {
						if (!isDirectAncestor(currentRefs.get(i), ancestor,
								refs, visited)) {
							return false;
						}
					}
	...
	public static boolean getPreviousReachableIps(int currentIp,
			HashMap<Integer, List<Integer>> refs, Set<Integer> reachable,
			Set<Integer> visited) {
		do {
			if (visited.contains(currentIp)) {
				return false;
			...
			if (refs.containsKey(currentIp)) {
				...
				if ((currentRefs != null) && (!currentRefs.isEmpty())) {
					...
					for (int i = 0; i < currentRefs.size(); i++) {
						...
						boolean v = getPreviousReachableIps(currentRefs.get(i),
								refs, nr, visited);
						if ((!v) || nr.contains(0)) {
							reachable.addAll(nr);
						}

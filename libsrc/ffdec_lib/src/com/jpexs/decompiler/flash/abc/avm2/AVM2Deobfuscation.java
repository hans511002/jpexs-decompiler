...
public class AVM2Deobfuscation {
	...
	private boolean isValidNSPart(String s) {
		...
		if (isValid) {
			Pattern pat = Pattern.compile("^([" + Pattern.quote(VALID_FIRST_CHARACTERS) + "]" + "["
					+ Pattern.quote(VALID_FIRST_CHARACTERS + VALID_NEXT_CHARACTERS + VALID_NS_CHARACTERS) + "]*)*$");
	...
	private String fooString(HashMap<DottedChain, DottedChain> deobfuscated, String orig, boolean firstUppercase,
			String usageType, RenameType renameType) {
		if (usageType == null) {
			usageType = "name";
		}
		...
		do {
			...
			if (swf.as3StringConstantExists(ret) || IdentifiersDeobfuscation.isReservedWord2(ret)
		...
		if (orig.matches(".*_\\d+")) {
			ret += orig.replaceFirst(".*(_\\d+)", "$1");
		}
		deobfuscated.put(DottedChain.parseWithSuffix(orig), DottedChain.parseWithSuffix(ret));
		return ret;
	}

	public int deobfuscatePackageName(Map<Integer, String> stringUsageTypes, Set<Integer> stringUsages,
			HashMap<DottedChain, DottedChain> namesMap, int strIndex, RenameType renameType) {
		if (strIndex <= 0) {
			return strIndex;
		}
	...
	public int deobfuscateName(Map<Integer, String> stringUsageTypes, Set<Integer> stringUsages,
			Set<Integer> namespaceUsages, HashMap<DottedChain, DottedChain> namesMap, int strIndex,
			boolean firstUppercase, RenameType renameType) {
		if (strIndex <= 0) {
			return strIndex;
		}
		String s = constants.getString(strIndex);
		...
		if (isValid) {
			Pattern pat = Pattern.compile("^[" + Pattern.quote(VALID_FIRST_CHARACTERS) + "]" + "["
					+ Pattern.quote(VALID_FIRST_CHARACTERS + VALID_NEXT_CHARACTERS) + "]*$");
			if (!pat.matcher(s).matches()) {
				isValid = false;
			}
		}
		...
		if (!isValid) {
			...
			if (namesMap.containsKey(sChain)) {
				...
				String str = fooString(namesMap, constants.getString(strIndex), firstUppercase,
						stringUsageTypes.get(strIndex), renameType);
				newname = DottedChain.parseWithSuffix(str);
			}
			if (stringUsages.contains(strIndex) || namespaceUsages.contains(strIndex)) { // this name is already

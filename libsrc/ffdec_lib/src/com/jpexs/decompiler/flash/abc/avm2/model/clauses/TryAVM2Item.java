...
public class TryAVM2Item extends AVM2Item implements Block {
	...
	public TryAVM2Item(List<GraphTargetItem> tryCommands,
			List<ABCException> catchExceptions,
			List<List<GraphTargetItem>> catchCommands,
			List<GraphTargetItem> finallyCommands, String finCatchName) {
	...
	public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData)
		...
		for (int e = 0; e < catchExceptions.size(); e++) {
			...
			String localName = catchExceptions.get(e).getVarName(
					localData.constantsAvm2, localData.fullyQualifiedNames);
			...
			data.declaredType = eti <= 0 ? DottedChain.ALL
					: localData.constantsAvm2
							.getMultiname(eti)
							.getNameWithNamespace(localData.constantsAvm2, true);
			nwriter.hilightSpecial(localName, HighlightSpecialType.TRY_NAME, e,
					data);
			nwriter.append(":");
			nwriter.hilightSpecial(
					catchExceptions.get(e).getTypeName(localData.constantsAvm2,

...
public class ASM3Parser {
	...
	public static AVM2Code parse(ABC abc, Reader reader, Trait trait,
			MethodBody body, MethodInfo info) throws IOException,
			AVM2ParseException, InterruptedException {
		return parse(abc, reader, trait, null, body, info);
	}

	private static int checkMultinameIndex(AVM2ConstantPool constants,
			int index, int line) throws AVM2ParseException {
	...
	private static void expected(int type, String expStr, Flasm3Lexer lexer)
			throws IOException, AVM2ParseException {
	...
	private static void expected(ParsedSymbol s, int type, String expStr)
			throws IOException, AVM2ParseException {
		if (s.type != type) {
	...
	private static void parseTraitParams(ABC abc, Flasm3Lexer lexer, Trait t)
			throws IOException, AVM2ParseException {
		...
		while (true) {
			...
			if (symb.type == ParsedSymbol.TYPE_KEYWORD_FLAG) {
				...
				case ParsedSymbol.TYPE_KEYWORD_METADATA:
					...
					break;
				default:
					throw new AVM2ParseException("Invalid trait flag",
							lexer.yyline());
				}
	...
	public static boolean parseSlotConst(ABC abc, Reader reader,
			AVM2ConstantPool constants, TraitSlotConst tsc) throws IOException,
			AVM2ParseException {
		...
		if (symb.type == ParsedSymbol.TYPE_KEYWORD_SLOT) {
			...
			throw new AVM2ParseException("slot or const expected",
					lexer.yyline());
		}
	...
	private static int parseNamespaceSet(AVM2ConstantPool constants,
			Flasm3Lexer lexer) throws AVM2ParseException, IOException {
		...
		loopn: for (int n = 1; n < constants.getNamespaceSetCount(); n++) {
	...
	private static int parseNamespace(AVM2ConstantPool constants,
			Flasm3Lexer lexer) throws AVM2ParseException, IOException {
		...
		case ParsedSymbol.TYPE_KEYWORD_STATICPROTECTEDNS:
			...
			break;
		default:
			throw new AVM2ParseException("Namespace kind expected",
					lexer.yyline());
		}
		...
		if (name.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
			...
			throw new AVM2ParseException("String or null expected",
					lexer.yyline());
		}
		...
		return constants.getNamespaceId(kind,
				name.type == ParsedSymbol.TYPE_KEYWORD_NULL ? null
						: (String) name.value, index, true);
	}

	private static int parseMultiName(AVM2ConstantPool constants,
			Flasm3Lexer lexer) throws AVM2ParseException, IOException {
		ParsedSymbol s = lexer.lex();
		int kind = 0;
		...
		case Multiname.QNAMEA: {
			...
			expected(ParsedSymbol.TYPE_PARENT_CLOSE, ")", lexer);
			multiname = Multiname.createQName(kind == Multiname.QNAMEA,
					name_index, namespace_index);
			break;
		}
		...
		case Multiname.RTQNAMEA: {
			...
			if (rtqName.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
				...
				name_index = constants
						.getStringId((String) rtqName.value, true);
			}
			expected(ParsedSymbol.TYPE_PARENT_CLOSE, ")", lexer);
			multiname = Multiname.createRTQName(kind == Multiname.RTQNAMEA,
					name_index);
			break;
		}
		...
		case Multiname.MULTINAMEA: {
			...
			expected(ParsedSymbol.TYPE_PARENT_CLOSE, ")", lexer);
			multiname = Multiname.createMultiname(kind == Multiname.MULTINAMEA,
					name_index, namespace_set_index);
			break;
		}
		...
		case Multiname.MULTINAMELA: {
			...
			int namespace_set_index = parseNamespaceSet(constants, lexer);
			expected(ParsedSymbol.TYPE_PARENT_CLOSE, ")", lexer);
			multiname = Multiname.createMultinameL(
					kind == Multiname.MULTINAMELA, namespace_set_index);
			break;
		}
	...
	public static ValueKind parseValue(AVM2ConstantPool constants,
			Flasm3Lexer lexer) throws IOException, AVM2ParseException {
		ParsedSymbol type = lexer.lex();
		ParsedSymbol value;
		int value_index = 0;
		int value_kind = 0;
		...
		case ParsedSymbol.TYPE_KEYWORD_DOUBLE:
			...
			break;
		/*
		 * case ParsedSymbol.TYPE_KEYWORD_DECIMAL: value_kind =
		 * ValueKind.CONSTANT_Decimal; break;
		 */
		...
		default:
			if (Configuration._debugMode.get()) {
				throw new AVM2ParseException("Not supported valueType.",
						lexer.yyline());
			}
		}
	...
	public static AVM2Code parse(ABC abc, Reader reader, Trait trait,
			MissingSymbolHandler missingHandler, MethodBody body,
			MethodInfo info) throws IOException, AVM2ParseException,
			InterruptedException {
		AVM2ConstantPool constants = abc.constants;
		AVM2Code code = new AVM2Code();
		int openedBlocks = 0;
		boolean autoCloseBlocks = true; // TODO? Put to false. But how about old
										// imports?
		...
		do {
			symb = lexer.lex();
			if (Arrays.asList(ParsedSymbol.TYPE_KEYWORD_BODY,
					ParsedSymbol.TYPE_KEYWORD_CODE,
					ParsedSymbol.TYPE_KEYWORD_METHOD).contains(symb.type)) {
				openedBlocks++;
				continue;
			}
			if (symb.type == ParsedSymbol.TYPE_KEYWORD_TRAIT) {
				openedBlocks++;
				if (trait == null) {
					throw new AVM2ParseException("No trait expected",
							lexer.yyline());
				}
				symb = lexer.lex();
				switch (symb.type) {
				...
				case ParsedSymbol.TYPE_KEYWORD_SETTER:
					if (!(trait instanceof TraitMethodGetterSetter)) {
						throw new AVM2ParseException("Unxpected trait type",
								lexer.yyline());
					}
			...
			if (symb.type == ParsedSymbol.TYPE_KEYWORD_NAME) {
				...
				if (symb.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
					...
					info.name_index = constants.getStringId(
							(String) symb.value, true);
				}
				continue;
			}
			...
			if (symb.type == ParsedSymbol.TYPE_KEYWORD_PARAMNAME) {
				...
				if (symb.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
					...
					expected(symb, ParsedSymbol.TYPE_STRING, "String or null");
					paramNames.add(constants.getStringId((String) symb.value,
							true));
				}
				continue;
			}
			...
			if (symb.type == ParsedSymbol.TYPE_EXCEPTION_START) {
				...
				if (listIndex == -1) {
					throw new AVM2ParseException("Undefinex exception index",
							lexer.yyline());
				}
			...
			if (symb.type == ParsedSymbol.TYPE_KEYWORD_END) {
				if (openedBlocks > 0) {
					...
					throw new AVM2ParseException(
							"End block encountered but there is no block opened",
							lexer.yyline());
				}
				continue;
			}
			if (symb.type == ParsedSymbol.TYPE_INSTRUCTION_NAME) {
				if (((String) symb.value).toLowerCase(Locale.ENGLISH).equals(
						"exception")) {
					ParsedSymbol exIndex = lexer.lex();
					if (exIndex.type != ParsedSymbol.TYPE_INTEGER) {
						throw new AVM2ParseException("Index expected",
								lexer.yyline());
					}
					ParsedSymbol exName = lexer.lex();
					if (exName.type != ParsedSymbol.TYPE_MULTINAME) {
						throw new AVM2ParseException("Multiname expected",
								lexer.yyline());
					}
					...
					ex.name_index = checkMultinameIndex(constants,
							(int) (long) (Long) exName.value, lexer.yyline());
					ex.type_index = checkMultinameIndex(constants,
							(int) (long) (Long) exType.value, lexer.yyline());
					exceptions.add(ex);
				...
				for (InstructionDefinition def : AVM2Code.instructionSet) {
					if (def != null && !(def instanceof UnknownInstruction)
							&& def.instructionName.equals((String) symb.value)) {
						insFound = true;
						...
						for (int i = 0; i < def.operands.length; i++) {
							...
							case AVM2Code.DAT_MULTINAME_INDEX:
								...
								operandsList.add(parseMultiName(constants,
										lexer));
								break;
							case AVM2Code.DAT_NAMESPACE_INDEX:
								lexer.pushback(parsedOperand);
								operandsList.add(parseNamespace(constants,
										lexer));
								break;
							case AVM2Code.DAT_STRING_INDEX:
								if (parsedOperand.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
									...
									int sid = constants
											.getStringId(
													(String) parsedOperand.value,
													false);
									if (sid == -1) {
										if ((missingHandler != null)
												&& (missingHandler
														.missingString((String) parsedOperand.value))) {
											sid = constants
													.addString((String) parsedOperand.value);
										} else {
											throw new AVM2ParseException(
													"Unknown String",
													lexer.yyline());
							...
							case AVM2Code.DAT_DOUBLE_INDEX:
								if (parsedOperand.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
									operandsList.add(0);
										...
										|| (parsedOperand.type == ParsedSymbol.TYPE_FLOAT)) {
							...
							case AVM2Code.DAT_FLOAT_INDEX:
								if (parsedOperand.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
									...
									int fid = constants.getFloatId(floatVal,
											false);
									if (fid == -1) {
										if ((missingHandler != null)
												&& (missingHandler
														.missingFloat(floatVal))) {
							...
							case AVM2Code.DAT_FLOAT4_INDEX:
								if (parsedOperand.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
									...
									int f4id = constants.getFloat4Id(float4Val,
											false);
									if (f4id == -1) {
										if ((missingHandler != null)
												&& (missingHandler
														.missingFloat4(float4Val))) {
											f4id = constants
													.addFloat4(float4Val);
										} else {
											throw new AVM2ParseException(
							...
							case AVM2Code.DAT_OFFSET:
								if (parsedOperand.type == ParsedSymbol.TYPE_IDENTIFIER) {
									offsetItems.add(new OffsetItem(
											(String) parsedOperand.value,
											code.code.size(), i));
									operandsList.add(0);
								} else {
									throw new AVM2ParseException(
											"Offset expected", lexer.yyline());
								}
								break;
							case AVM2Code.DAT_CASE_BASEOFFSET:
								if (parsedOperand.type == ParsedSymbol.TYPE_IDENTIFIER) {
									offsetItems.add(new CaseOffsetItem(
											(String) parsedOperand.value,
											code.code.size(), i));
									operandsList.add(0);
								} else {
									...
									if (val < Byte.MIN_VALUE
										...
										throw new AVM2ParseException(
												"Byte value expected ("
														...
														+ " to "
														+ Byte.MAX_VALUE
														+ "). Use pushshort or pushint to push larger values",
												lexer.yyline());
									}
									operandsList.add((int) val);
								} else {
									throw new AVM2ParseException(
											"Integer expected", lexer.yyline());
								}
								break;
							default:
								if (parsedOperand.type == ParsedSymbol.TYPE_INTEGER) {
									...
									if (def instanceof PushShortIns) {
										if (val < Short.MIN_VALUE
												|| val > Short.MAX_VALUE) {
											throw new AVM2ParseException(
													"Short value expected ("
															+ Short.MIN_VALUE
															+ " to "
															+ Short.MAX_VALUE
															+ "). Use pushint to push larger values",
													lexer.yyline());
										}
									}
									operandsList.add((int) val);
				...
				if (symb.value.toString().toLowerCase()
						.equals("ffdec_deobfuscatepop")) {
					lastIns = new AVM2Instruction(offset,
							DeobfuscatePopIns.getInstance(), null);
					code.code.add(lastIns);
					offset += lastIns.getBytesLength();
					insFound = true;
				}
				if (!insFound) {
					throw new AVM2ParseException("Invalid instruction name:"
							+ (String) symb.value, lexer.yyline());
				}
			} else if (symb.type == ParsedSymbol.TYPE_LABEL) {
				labelItems.add(new LabelItem((String) symb.value, offset));

			} else {
				throw new AVM2ParseException("Unexpected symbol",
						lexer.yyline());
			}
		} while (symb.type != ParsedSymbol.TYPE_EOF);

		if (!autoCloseBlocks && openedBlocks > 0) {
			throw new AVM2ParseException("End of the block expected: "
					+ openedBlocks + "x", lexer.yyline());
		}

...
public class SlotConstTraitDetailPanel extends JPanel implements TraitDetail {
	...
	public SlotConstTraitDetailPanel(final DecompiledEditorPane editor) {
		...
		slotConstEditor.addCaretListener(new CaretListener() {
			...
			public void caretUpdate(CaretEvent e) {
				...
				Highlighting spec = Highlighting.searchPos(specialHilights,
						slotConstEditor.getCaretPosition());
				if (spec != null) {
					editor.hilightSpecial(spec.getProperties().subtype,
							(int) spec.getProperties().index);
	...
	public void hilightSpecial(Highlighting special) {
		...
		for (Highlighting h : specialHilights) {
			if (h.getProperties().subtype
					.equals(special.getProperties().subtype)) {
	...
	public boolean save() {
		try {// (slotConstEditor.getText(), trait, abc)
			if (!ASM3Parser.parseSlotConst(abc, new StringReader(
					slotConstEditor.getText()), abc.constants, trait)) {
				return false;
			}
		} catch (AVM2ParseException ex) {
			View.showMessageDialog(slotConstEditor, ex.text,
					AppStrings.translate("error.slotconst.typevalue"),
					JOptionPane.ERROR_MESSAGE);
			return false;
		} catch (IOException ex) {
			Logger.getLogger(SlotConstTraitDetailPanel.class.getName()).log(
	...
	public void setEditMode(boolean val) {
		if (val && active) {
			JOptionPane.showMessageDialog(null,
					AppStrings.translate("warning.initializers"),
